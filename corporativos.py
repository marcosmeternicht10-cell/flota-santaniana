...
def semanas_liquidables(chofer_usuario):
    ...
    conn.close()
    if u:
        tiene_plus = (u["nombre"] or "").strip().upper() in [c.upper() for c in CHOFERES_CON_PLUS]
    ...
    for sem, g in sorted(grupos.items()):
        ...
        plus = PLUS_SEMANAL if (tiene_plus and g["trabajo"]) else 0
        salida.append({
...
def liberar_monto_rendicion(rid):
    ...
    conn = get_connection()
    row = conn.execute("""SELECT tramo, estado, liquidado FROM rendiciones_corp
                          WHERE id=?""", (int(rid),)).fetchone()
    if not row:
        conn.close()
        return False, "No encontré esa rendición."
    ...
    monto = calcular_monto(row["tramo"], row["estado"])
    conn.execute("""UPDATE rendiciones_corp
