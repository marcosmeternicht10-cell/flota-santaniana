...
def control_cobertura(cliente, desde, hasta):
    """Qué tramos se esperaban y cuáles se reportaron realmente.

    Un tramo se considera CUBIERTO si alguien lo marcó completado, o si entre
    dos choferes se cubrieron el entrante y el saliente. Si solo hay una de las
    dos mitades queda PARCIAL. Si no hay nada, FALTANTE.
    """
    fijos = tramos_fijos(cliente)
    dias = dias_operativos(cliente, desde, hasta)
    if not fijos or not dias:
        return {"cliente": cliente, "esperados": 0, "cubiertos": 0,
                "parciales": 0, "faltantes": 0, "suspendidos": 0,
                "cobertura": 0, "detalle_faltantes": [], "dias": 0}
    ...
    """, (cliente, desde, hasta)).fetchall()]
    conn.close()

    # Lo reportado, indexado por día y tramo
    reportado = {}
    for r in rows:
        if _es_variable(r["tramo"]):
            continue
        clave = (r["fecha_servicio"], r["tramo"])
        reportado.setdefault(clave, []).append(r)

    cubiertos = parciales = faltantes = suspendidos = 0
    detalle = []
    for dia in dias:
        for tramo in fijos:
            ...
            if "completado" in estados or ("entrante" in estados and "saliente" in estados):
                ...
                suspendidos += 1
                obs = next((x.get("chofer_nombre") for x in regs), "")
                detalle.append({"fecha": dia, "tramo": tramo, "situacion": "suspendido",
                                "detalle": "servicio suspendido", "quien": obs})
            else:
                faltantes += 1
                detalle.append({"fecha": dia, "tramo": tramo, "situacion": "faltante",
                                "detalle": "nadie lo reportó", "quien": "—"})

    esperados = len(dias) * len(fijos)
    return {
        "cliente": cliente, "dias": len(dias), "tramos_por_dia": len(fijos),
        "esperados": esperados, "cubiertos": cubiertos, "parciales": parciales,
        "faltantes": faltantes, "suspendidos": suspendidos,
        "cobertura": round(cubiertos / esperados * 100, 1) if esperados else 0,
...
def costo_por_empresa(desde=None, hasta=None, agrupacion="total"):
    ...
    for r in rows:
        cli = r["cliente"]
        per = periodo_de(r["fecha_servicio"])
...
def variables_del_periodo(desde=None, hasta=None, cliente=None):
    ...
    sin_justificar = [r for r in variables if not (r.get("observacion") or "").strip()]
...
def api_corp_control_empresa():
    ...
    if desde and hasta:
        for c in objetivo:
            cobertura.append(control_cobertura(c, desde, hasta))
...
def api_corp_control_empresa_pdf():
    ...
    activos = [c for c in CLIENTES_TRAMOS if c not in CLIENTES_INACTIVOS]
    objetivo = [cliente] if cliente else activos
    cobertura = [control_cobertura(c, desde, hasta) for c in objetivo] if (desde and hasta) else []
...
def generar_pdf_control_empresa(cobertura, costos, variables, desde, hasta):
    ...
    if cobertura:
        ...
        d = [["Empresa", "Días", "Esperados", "Cubiertos", "Parciales",
              "Faltantes", "Cobertura"]]
        for c in cobertura:
            d.append([c["cliente"], str(c["dias"]), str(c["esperados"]),
                      str(c["cubiertos"]), str(c["parciales"]),
                      str(c["faltantes"]), f'{c["cobertura"]}%'])
        elems.append(tabla(d, [38*mm, 16*mm, 24*mm, 24*mm, 22*mm, 22*mm, 24*mm]))
    ...
    if v.get("cantidad"):
        ...
        for x in v["variables"]:
            d3.append([x["fecha_servicio"], x["cliente"],
                       ...
                       (x.get("observacion") or "SIN JUSTIFICAR")[:46],
                       gs(x["monto"])])
