...
def fecha_reportable(fecha_servicio):
    """Si esa fecha se puede reportar hoy.

    La ventana es el día del servicio y el siguiente. No se admiten fechas
    futuras: nadie puede reportar un viaje que todavía no hizo.
    """
    ...
    if dias > DIAS_ATRAS_PERMITIDOS:
        return False, (f"Ese servicio es de hace {dias} días. Se puede reportar "
                       f"el mismo día o al día siguiente; para algo más viejo "
                       f"hablá con el encargado.")
    return True, ""
...
def puede_cargar_servicio(chofer_usuario, fecha, tramo, hora_inicio="",
    ...
    """Si el chofer puede sumar este servicio a su día.

    Un servicio suspendido no ocupa tiempo, así que siempre se puede reportar.
    Los variables tampoco se controlan por franja: son justamente lo que se
    hace fuera de la grilla. Pero sí cuentan para el máximo del día.
    """
    if estado == "suspendido":
        return True, ""
    ...
    if len(ya) >= MAX_SERVICIOS_DIA:
        return False, (f"Ya tenés {len(ya)} servicios cargados ese día, que es "
                       f"el máximo. Si alguno está mal, avisale al encargado.")

    nueva = franja_del_servicio(tramo, hora_inicio)
    if nueva is None or _es_variable(tramo):
        return True, ""
    ...
    for s in ya:
        ...
        if otra is None:
            continue
        choca = (nueva == "completa" or otra == "completa" or nueva == otra)
        if choca:
            detalle = f"{s['cliente']} · {s['tramo']}"
            if otra == "completa":
                return False, (f"Ese día ya hiciste un servicio de jornada completa "
                               f"({detalle}). No podés sumar otro.")
            if nueva == "completa":
                return False, (f"Este servicio ocupa todo el día y ya tenés otro "
                               f"cargado ({detalle}).")
            return False, (f"Ya tenés un servicio en {NOMBRE_FRANJA[otra]} ese día "
                           f"({detalle}). El segundo servicio tiene que ser en "
                           f"{NOMBRE_FRANJA['tarde' if otra == 'manana' else 'manana']}.")
...
def registrar_rendicion(datos, limitar_fecha=False):
    ...
    # ¿El chofer puede hacer este servicio ese día? Nadie está en dos lugares
    # a la vez: dos servicios en la misma franja no pueden ser.
    libre, motivo = puede_cargar_servicio(
        datos["chofer_usuario"], datos["fecha_servicio"],
        datos["tramo"], datos.get("hora_inicio", ""), estado)
    if not libre:
        return False, motivo
...
def rendiciones_sospechosas(desde=None, hasta=None):
    """Servicios que conviene mirar de cerca antes de pagarlos.

    Marca tres cosas distintas, porque no todas significan lo mismo:
      - fecha futura: imposible, el servicio todavía no pasó
      - cargado tarde: se reportó varios días después del servicio, cuando ya
        nadie se acuerda bien de lo que hizo
      - en bloque: varios servicios de días distintos cargados en el mismo
        momento, que es la firma de alguien completando la planilla de memoria

    No borra nada: junta lo que hay para que se revise y se decida.
    """
    import datetime as _dt
    q = """SELECT id, chofer_usuario, chofer_nombre, cliente, tramo,
                  fecha_servicio, estado, monto, liquidado,
                  COALESCE(cargado_el,'') AS cargado_el
           FROM rendiciones_corp WHERE COALESCE(liquidado,0)=0"""
    params = []
    if desde:
        q += " AND fecha_servicio>=?"; params.append(desde)
    if hasta:
    ...
    for r in rows:
        try:
            ...
            continue
        if f > hoy_py:
            r["motivo"] = f"fecha futura ({(f - hoy_py).days} día/s adelante)"
            futuras.append(r)
            continue
        ...
        if cargado:
            try:
                cf = _dt.date.fromisoformat(cargado)
                dias = (cf - f).days
                if dias > DIAS_ATRAS_PERMITIDOS:
    ...
    en_bloque.sort(key=lambda x: -x["cantidad"])

    sin_registro = sum(1 for r in rows if not (r.get("cargado_el") or "").strip())
    return {
        "futuras": futuras,
        "tardias": sorted(tardias, key=lambda x: -x.get("dias_demora", 0)),
        "en_bloque": en_bloque,
        "sin_registro_de_carga": sin_registro,
        "total_revisar": len(futuras) + len(tardias),
    }
...
def api_corp_cargar_combustible():
    ...
    if odo <= 0:
        return jsonify({"ok": False, "msg": "Poné el kilometraje del tablero."}), 400
...
def generar_pdf_combustible_corp(cargas, resumen, desde, hasta, agrupacion="total"):
    ...
    t = Table(tot, colWidths=[40*mm, 44*mm, 48*mm, 50*mm])
    ...
    # ── Por período ──
    if agrupacion != "total" and len(r["por_periodo"]) > 1:
        etiqueta = {"dia": "día", "semana": "semana", "mes": "mes"}.get(agrupacion, "período")
        elems.append(Paragraph(f"Por {etiqueta}", st_sec))
        d = [["Período", "Cargas", "Litros", "Costo Gs."]]
        for p in r["por_periodo"]:
            d.append([p["periodo"], str(p["cargas"]), lt(p["litros"]), gs(p["costo"])])
        d.append(["TOTAL", str(r["cargas"]), lt(r["litros"]), gs(r["costo"])])
        elems.append(tabla(d, [72*mm, 30*mm, 38*mm, 42*mm], ultima=True))

    # ── Por chofer ──
    elems.append(Paragraph("Por chofer", st_sec))
    d = [["Chofer", "Cargas", "Litros", "Costo Gs."]]
    for x in r["por_chofer"]:
        d.append([x["chofer"], str(x["cargas"]), lt(x["litros"]), gs(x["costo"])])
    elems.append(tabla(d, [72*mm, 30*mm, 38*mm, 42*mm]))

    # ── Por bus ──
    elems.append(Paragraph("Por bus", st_sec))
    d = [["Bus", "Patente", "Cargas", "Litros", "Km/L", "Costo Gs."]]
    for x in r["por_bus"]:
        d.append([f'#{x["bus"]}', x.get("patente", ""), str(x["cargas"]),
                  lt(x["litros"]), (str(x["rendimiento"]) if x["rendimiento"] else "—"),
                  gs(x["costo"])])
    elems.append(tabla(d, [26*mm, 32*mm, 24*mm, 32*mm, 24*mm, 44*mm]))

    # ── Detalle ──
    elems.append(Paragraph(f"Detalle de las cargas ({len(cargas)})", st_sec))
    d = [["Fecha", "Bus", "Chofer", "Odómetro", "Litros", "Gs./L", "Total Gs."]]
    for c in cargas[:90]:
        d.append([c.get("fecha", ""),
                  f'#{c.get("n_interno") or ""}',
                  (c.get("chofer") or "")[:20],
                  gs(c.get("odometro")), lt(c.get("litros")),
                  gs(c.get("precio_litro")), gs(c.get("costo_total"))])
    t2 = tabla(d, [22*mm, 18*mm, 40*mm, 26*mm, 22*mm, 22*mm, 32*mm])
