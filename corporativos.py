...
def inicializar_corporativos():
    ...
    try:
        ...
        for col, ddl in (("liquidado", "INTEGER DEFAULT 0"),
                         ...
                         # después
                         ("cargado_el", "TEXT"),
                         # Cuando el servicio choca en horario con otro del
                         # mismo chofer y él explica por qué, queda acá
                         ("justificacion_horario", "TEXT DEFAULT ''"),
...
def rango_horario(tramo, hora_inicio="", hora_fin="", estado="completado"):
    ...
    Si el chofer hizo solo una mitad de una jornada larga, ocupa solo esa
    punta del día: el entrante es el viaje que termina a la hora de entrada
    (7:00 → de 6:00 a 7:30) y el saliente el que arranca a la hora de salida
    (17:00 → de 16:30 a 18:00). El que hizo solo la salida de las 17 no
    estaba en la planta a las 14, y puede haber hecho otro servicio antes.
    """
    horas = _horas_del_tramo(tramo)
    if len(horas) >= 2:
    ...
    if estado in ("entrante", "saliente") and (fin - ini) >= HORAS_JORNADA_COMPLETA:
        if estado == "entrante":
            return (ini - 1.0, ini + 0.5)
        return (fin - 0.5, fin + 1.0)
    return (ini, fin)
...
def puede_cargar_servicio(chofer_usuario, fecha, tramo, hora_inicio="",
    ...
    Es el único control de horario que queda, y se limita al choque real:
    nadie puede hacer dos tramos a la misma hora. Dos servicios seguidos o en
    momentos distintos del día pasan sin problema; tampoco hay tope de
    cantidad de servicios ni límite de fechas hacia atrás.
    """
    ...
    nuevo = rango_horario(tramo, hora_inicio, estado=estado)
    if not nuevo:
        return True, ""
    ...
    for s in ya:
        if _es_variable(s["tramo"]):
            continue
        otro = rango_horario(s["tramo"], s.get("hora_inicio", ""), estado=s.get("estado") or "completado")
        if _se_pisan(nuevo, otro):
            return False, (f"Ese día, a esa misma hora, ya hay cargado: "
                           f"{s['cliente']} · {s['tramo']} "
                           f"({_hhmm(otro[0])} a {_hhmm(otro[1])}). "
                           f"No se puede estar en dos lugares a la vez: "
                           f"si el que está cargado es el equivocado, corregilo o borralo primero.")
    return True, ""
...
def registrar_rendicion(datos, limitar_fecha=False):
    ...
    # Si choca en horario con otro servicio del mismo chofer, no se rebota
    # sin más: en el papel dos tramos pueden caer a la misma hora y en la
    # realidad haberse hecho uno atrás del otro (terminó la salida de un
    # cliente y fue directo al otro). Se le pide que lo explique por escrito:
    # con eso entra, y queda marcado para que el encargado lo revise.
    justificacion = (datos.get("justificacion_horario") or "").strip()
    libre, motivo = puede_cargar_servicio(
        datos["chofer_usuario"], datos["fecha_servicio"], datos["tramo"],
        datos.get("hora_inicio", ""), estado)
    if not libre:
        if len(justificacion) < JUSTIFICACION_MIN:
            return False, {"msg": motivo, "requiere_justificacion": True,
                           "minimo": JUSTIFICACION_MIN}
    else:
        justificacion = ""   # si no choca, no hay nada que justificar
    ...
    try:
        conn.execute("""INSERT INTO rendiciones_corp
            (chofer_usuario, chofer_nombre, cliente, tramo, fecha_servicio,
             ...
             pasajeros, estado, observacion, monto, cargado_el, justificacion_horario)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (datos.get("chofer_usuario", ""), datos.get("chofer_nombre", ""),
             ...
             datos.get("observacion", "").strip(), monto, ahora_iso(), justificacion))
        conn.commit()
...
def _pares_que_se_pisan(grupo):
    """Qué servicios del grupo se pisan de verdad en el horario.

    Se compara el horario real de cada tramo, no una franja gruesa: una
    jornada de 7:30 a 17:00 y un turnante de 23:50 no se pisan, pero dos
    turnantes a la misma hora sí. Devuelve los ids involucrados y el detalle
    de cada cruce.
    """
    rangos = []
    for g in grupo:
        if _es_variable(g["tramo"]):
            continue
        r = rango_horario(g["tramo"], g.get("hora_inicio", ""),
                          estado=g.get("estado") or "completado")
        if r:
            rangos.append((g, r))
    ...
    for i in range(len(rangos)):
        for j in range(i + 1, len(rangos)):
            (a, ra), (b, rb) = rangos[i], rangos[j]
            if _se_pisan(ra, rb):
                ids.add(a["id"])
                ids.add(b["id"])
                detalle.append(f"{a['tramo']} ({hhmm(ra[0])}) se pisa con "
                               f"{b['tramo']} ({hhmm(rb[0])})")
    return ids, detalle
...
def rendiciones_sospechosas(desde=None, hasta=None):
    ...
    rows = [dict(r) for r in conn.execute("""
        SELECT id, chofer_usuario, chofer_nombre, cliente, tramo, bus_interno,
               ...
               COALESCE(justificacion_horario,'') AS justificacion_horario
        FROM rendiciones_corp
    ...
    for (usuario, fecha), grupo in por_chofer_dia.items():
        ...
        # 5. Dos servicios del mismo chofer a la misma hora
        ids_choque, detalle_choque = _pares_que_se_pisan(grupo)
        if ids_choque:
            choque = [g for g in grupo if g["id"] in ids_choque]
            # Lo que explicó el chofer al cargar, si lo explicó
            justif = [f"{g['tramo']}: «{g['justificacion_horario']}»"
                      for g in choque if (g.get("justificacion_horario") or "").strip()]
            misma_franja.append({
                "chofer_usuario": usuario, "chofer_nombre": grupo[0]["chofer_nombre"],
                "fecha": fecha, "cantidad": len(choque),
                "ids": [g["id"] for g in choque],
                "monto": round(sum(float(g["monto"] or 0) for g in choque)),
                "detalle": detalle_choque,
                "justificaciones": justif,
                "justificado": bool(justif),
            })
    ...
    for (bus, fecha), grupo in por_bus_dia.items():
        ...
        ids_bus, _ = _pares_que_se_pisan(grupo)
        if ids_bus and len(choferes) > 1:
            cruzados = [g for g in grupo if g["id"] in ids_bus]
            bus_compartido.append({
                "bus": bus, "fecha": fecha, "cantidad": len(cruzados),
                "ids": [g["id"] for g in cruzados],
                "choferes": sorted({g["chofer_nombre"] for g in cruzados}),
                "detalle": [f"{g['chofer_nombre']}: {g['tramo']}" for g in cruzados],
                "monto": round(sum(float(g["monto"] or 0) for g in cruzados)),
            })
...
def api_corp_rendicion():
    ...
    if isinstance(msg, dict):          # choca en horario: falta la explicación
        return jsonify({"ok": False, **msg}), 409
...
@bp_corp.route("/api/corp/combustible_bus_pdf", methods=["GET"])
def api_corp_combustible_bus_pdf():
    """Informe de consumo por coche, para imprimir."""
    if session.get("rol") not in ("admin", "auditor", "taller"):
        return jsonify({"error": "Sin permiso"}), 403
    from flask import send_file
    from database import obtener_cargas_combustible
    ...
    cargas = obtener_cargas_combustible(desde=desde, hasta=hasta,
                                        origen="corporativo", chofer=chofer,
                                        limite=5000)
    pdf = generar_pdf_por_bus(resumen_combustible_corp(cargas, "total"),
                              desde or "", hasta or "", chofer or "", cargas)
    return send_file(io.BytesIO(pdf), mimetype="application/pdf",
                     download_name=f"consumo_por_coche_{desde or ''}_{hasta or ''}.pdf")


def generar_pdf_por_bus(resumen, desde, hasta, chofer="", cargas=None):
    """Informe de consumo por coche.

    Pensado para imprimir y repartir: una franja de encabezado, los números
    grandes arriba y una fila por coche con su barra, para que se lea de un
    vistazo cuál gasta más sin tener que comparar números a mano.
    """
    ...
    NOCHE = colors.HexColor("#123A63")
    TEXTO = colors.HexColor("#1A1A18")
    GRIS = colors.HexColor("#6E6D66")
    CLARO = colors.HexColor("#A5A49E")
    LINEA = colors.HexColor("#ECEAE5")
    SUAVE = colors.HexColor("#F7F6F3")
    VERDE = colors.HexColor("#2F7C46")
    AMBAR = colors.HexColor("#B97D0A")
    ROJO = colors.HexColor("#DC2641")
    CELESTE = colors.HexColor("#1E5A96")

    S = getSampleStyleSheet()
    def est(nombre, size, color, **kw):
        kw.setdefault("leading", size * 1.25)
        return ParagraphStyle(nombre, parent=S["Normal"], fontSize=size,
                              textColor=color, **kw)

    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4, leftMargin=14*mm, rightMargin=14*mm,
                            topMargin=13*mm, bottomMargin=16*mm,
                            title="Consumo por coche")
    ANCHO = A4[0] - 28*mm

    def lt(v, dec=1):
        return f'{(v or 0):,.{dec}f}'.replace(",", "@").replace(".", ",").replace("@", ".")
    ...
    izq = [Paragraph("CONSUMO POR COCHE",
                     est("h1", 19, colors.white, fontName="Helvetica-Bold", leading=22)),
           Paragraph("La Santaniana · Gestión de flota",
                     est("h2", 9, colors.HexColor("#A9C6E3")))]
    def dmy(f):
        try:
            return datetime.date.fromisoformat(f).strftime("%d/%m/%Y")
        except Exception:
            return f or "—"
    periodo = f"{dmy(desde)}  al  {dmy(hasta)}"
    der = [Paragraph(periodo, est("d1", 9.5, colors.white, alignment=2,
                                  fontName="Helvetica-Bold")),
           Paragraph((f"Chofer: {chofer}" if chofer else "Toda la flota"),
                     est("d2", 8.5, colors.HexColor("#A9C6E3"), alignment=2))]
    cab = Table([[izq, der]], colWidths=[ANCHO * 0.62, ANCHO * 0.38])
    cab.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), NOCHE),
    ...
    r = resumen
    buses = r["por_bus"]
    con_consumo = [b for b in buses if b.get("litros_100km")]
    # Promedio de la flota sobre los kilómetros reales, no promediando coches:
    # uno que hizo 300 km no puede pesar igual que uno que hizo 5.000.
    km_tot = sum((x.get("km_recorridos") or 0) for x in (cargas or []) if x.get("consumo_valido"))
    lit_tot = sum((x.get("litros_del_tramo") or 0) for x in (cargas or []) if x.get("consumo_valido"))
    if km_tot:
        prom = lit_tot / km_tot * 100
    else:
        prom = (sum(b["litros_100km"] for b in con_consumo) / len(con_consumo)) if con_consumo else 0

    def kpi(valor, etiqueta, color):
        t = Table([[Paragraph(valor, est("kv", 17, TEXTO, fontName="Helvetica-Bold"))],
                   [Paragraph(etiqueta, est("kl", 8, GRIS))]],
                  colWidths=[(ANCHO - 18) / 4])
    ...
    tira = Table([[kpi(str(len(buses)), "coches con cargas", AZUL),
                   kpi(nm(r["cargas"]), "cargas del período", AZUL),
                   kpi(lt(r["litros"]) + " L", "litros cargados", AMBAR),
                   kpi(lt(prom, 2) if prom else "—", "consumo promedio · L/100km", VERDE)]],
                 colWidths=[ANCHO / 4] * 4)
    tira.setStyle(TableStyle([("LEFTPADDING", (0, 0), (-1, -1), 0),
                              ("RIGHTPADDING", (0, 0), (-1, -1), 6),
                              ("VALIGN", (0, 0), (-1, -1), "TOP")]))
    e += [tira, Spacer(1, 14)]

    # ── Una fila por coche ──
    e.append(Paragraph("Coches", est("sec", 11.5, AZUL, fontName="Helvetica-Bold",
                                     spaceAfter=5)))
    W_BARRA = 38*mm
    tope = max([b["litros_100km"] for b in con_consumo], default=1)

    def barra(valor, color):
        d = Drawing(W_BARRA, 8)
        d.add(Rect(0, 1.5, W_BARRA, 5, fillColor=LINEA, strokeColor=None))
        w = max(2.0, W_BARRA * min(1.0, (valor or 0) / tope))
        d.add(Rect(0, 1.5, w, 5, fillColor=color, strokeColor=None))
        return d

    # Ordenados por número de coche. Los internos no siempre son números
    # (está el "H1 03"), así que los que tienen letras van al final, ordenados
    # alfabéticamente.
    def orden_coche(b):
        n = str(b.get("bus") or "").strip()
        return (0, int(n), "") if n.isdigit() else (1, 0, n.upper())
    buses = sorted(buses, key=orden_coche)

    enc = ["Coche", "Patente", "Cargas", "Litros", "Consumo\nL/100km", "", "Vs. su motor"]
    datos = [enc]
    estilos = [
        ("BACKGROUND", (0, 0), (-1, 0), NOCHE),
        ...
        ("LINEBELOW", (0, 1), (-1, -2), 0.5, LINEA),
        ("FONTSIZE", (0, 1), (-1, -1), 9),
    ]
    for i, b in enumerate(buses, start=1):
        comp = b.get("comparacion") or {}
        if comp.get("sin_datos"):
            vs, col = {"sin_plan": "falta el plan",
                       "sin_consumo": "faltan cargas"}.get(comp.get("estado"), "—"), CLARO
        elif comp:
            vs = f'{"+" if comp["desvio_pct"] > 0 else ""}{comp["desvio_pct"]}%'
            col = {"bien": VERDE, "atencion": AMBAR, "malo": ROJO,
                   "revisar": CELESTE}.get(comp.get("estado"), GRIS)
        else:
            vs, col = "—", CLARO
        cons = b.get("litros_100km")
        datos.append([
            Paragraph(f'<b>#{b["bus"]}</b>', est("bn", 10, TEXTO)),
            Paragraph(b.get("patente", "") or "", est("bp", 8.5, GRIS)),
            str(b["cargas"]), lt(b["litros"]),
            Paragraph(f'<b>{lt(cons, 2)}</b>' if cons else '—', est("bc", 10, TEXTO, alignment=1)),
            barra(cons, col) if cons else "",
            Paragraph(vs, est("bv", 8.5, col, alignment=1, fontName="Helvetica-Bold")),
        ])
        if i % 2 == 0:
            estilos.append(("BACKGROUND", (0, i), (-1, i), SUAVE))
    t = Table(datos, repeatRows=1,
              colWidths=[24*mm, 26*mm, 16*mm, 24*mm, 24*mm, W_BARRA, 30*mm])
    t.setStyle(TableStyle(estilos))
    e.append(t)

    # ── Cómo leerlo ──
    e.append(Spacer(1, 10))
    ley = [[Paragraph("<b>Cómo leer este informe</b>", est("ly", 8.5, TEXTO))],
           [Paragraph("El consumo se mide de tanque a tanque: los kilómetros recorridos "
                      "sobre los litros que hizo falta cargar para volver a llenar. "
                      "La barra compara cada coche contra el que más gasta.",
                      est("ly2", 8, GRIS))],
           [Paragraph('<font color="#2F7C46"><b>Verde</b></font>: dentro de lo esperado para su motor. &nbsp;&nbsp;'
                      '<font color="#B97D0A"><b>Ámbar</b></font>: algo por encima. &nbsp;&nbsp;'
                      '<font color="#DC2641"><b>Rojo</b></font>: bastante por encima, conviene revisarlo. &nbsp;&nbsp;'
                      '<font color="#1E5A96"><b>Azul</b></font>: consumió menos de lo esperado, suele ser un kilometraje mal cargado.',
                      est("ly3", 8, GRIS))],
           [Paragraph('"Faltan cargas": hacen falta al menos dos cargas seguidas con kilometraje para poder medirlo. '
                      '"Falta el plan": el coche no tiene plan de mantenimiento asignado, que es de donde sale el consumo esperado.',
                      est("ly4", 8, GRIS))]]
    tl = Table(ley, colWidths=[ANCHO])
    tl.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), SUAVE),
    ...
    def pie(canvas, documento):
        ...
        canvas.setFillColor(CLARO)
        canvas.drawString(14*mm, 7*mm,
                          f"La Santaniana · generado el {datetime.datetime.now().strftime('%d/%m/%Y')}")
        canvas.drawRightString(A4[0] - 14*mm, 7*mm, f"Página {documento.page}")
        canvas.restoreState()
