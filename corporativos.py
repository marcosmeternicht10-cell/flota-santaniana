...
def rendiciones_sospechosas(desde=None, hasta=None):
    ...
    rows = [dict(r) for r in conn.execute("""
        SELECT id, chofer_usuario, chofer_nombre, cliente, tramo, bus_interno,
               fecha_servicio, hora_inicio, estado, monto, liquidado,
    ...
    # 7. Varios días cargados en el mismo minuto
    en_bloque = []
    ...
    total = (len(futuras) + len(tardias) + len(dia_imposible) +
             len(misma_franja) + len(bus_compartido) + len(no_laborable) + len(en_bloque))
    return {
        "futuras": futuras,
