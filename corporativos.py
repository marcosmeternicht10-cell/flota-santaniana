...
def api_corp_cargar_combustible():
    ...
    cid = registrar_carga_combustible({
        ...
        "precio_litro": num(d.get("precio_litro")),
        "costo_total": num(d.get("costo_total")),
        "estacion": (d.get("estacion") or "").strip(),
        "chofer": nombre,
        "tanque_lleno": bool(d.get("tanque_lleno", True)),
...
def resumen_combustible_corp(cargas, agrupacion="total"):
    ...
    for b in por_bus.values():
        r = b.pop("rendimientos", [])
        b["rendimiento"] = round(sum(r) / len(r), 2) if r else 0
        b["litros"] = round(b["litros"], 1); b["costo"] = round(b["costo"])
    for d in list(por_periodo.values()) + list(por_chofer.values()):
        d["litros"] = round(d["litros"], 1); d["costo"] = round(d["costo"])
