...
def control_cobertura(cliente, desde, hasta, agrupacion="total"):
    ...
    El resultado viene desglosado según la agrupación elegida: pidiendo por
    semana se ve el rendimiento de cada semana por separado, no el total del
    rango mezclado.
    """
    ...
    # Se acumula por período y en total
    periodos = {}
    detalle = []
    tot = {"esperados": 0, "cubiertos": 0, "parciales": 0,
           "faltantes": 0, "suspendidos": 0, "dias": len(dias)}
...
def api_corp_control_empresa():
    ...
    if desde and hasta:
        for c in objetivo:
            cobertura.append(control_cobertura(c, desde, hasta, agrup))
...
def api_corp_control_empresa_pdf():
    ...
    activos = [c for c in CLIENTES_TRAMOS if c not in CLIENTES_INACTIVOS]
    objetivo = [cliente] if cliente else activos
    cobertura = [control_cobertura(c, desde, hasta, agrup) for c in objetivo] if (desde and hasta) else []
