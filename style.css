...
async function api(url, opts) {
  ...
  // Sin permiso (rol consulta intentando modificar)
  if (r.status === 403) {
      ...
      ` : `
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Patente</th><th>Marca / Modelo</th><th>Motor / Plan</th>
...
function renderOdometro(p) {
  return `<div class="odometer-card">
    <div>
      ...
      <div class="odo-value">${fmtNum(p.km_actual)} km</div>
      <div class="odo-plan"><i class="ti ti-engine"></i> Plan: <b>${p.plan.plan_nombre}</b></div>
    </div>
...
async function renderCorrectivos() {
  ...
  content.innerHTML = `
    <div class="page-header">
      ...
      <p>Registro de averías y reparaciones no planificadas</p>
    </div>
    <div class="section">
      <div class="card">
      ...
      <div class="section-title"><i class="ti ti-list"></i> Historial de correctivos (${correctivos.length})</div>
      ${correctivos.length === 0 ? `
        <div class="empty"><i class="ti ti-mood-smile"></i>Sin correctivos registrados.<br>¡Buena señal!</div>
      ` : `
        <div class="table-wrap">
          <table>
            <thead><tr>
...
async function renderDocumentos() {
  ...
  content.innerHTML = `
    <div class="page-header">
      ...
      <p>Control de VTV, seguros, habilitaciones y otros vencimientos</p>
    </div>
    <div class="section">
      <div class="card">
      ...
      <div class="section-title"><i class="ti ti-calendar"></i> Documentos registrados (${docs.length})</div>
      ${docs.length === 0 ? `<div class="empty"><i class="ti ti-file-off"></i>Sin documentos registrados</div>` : `
        <div class="table-wrap">
          <table>
            <thead><tr>
            ...
            <tbody>
              ${docs.map(d => {
                const dias = diasHasta(d.fecha_vencimiento);
                let estadoBadge, icono;
                if (dias < 0) { estadoBadge = `<span class="badge danger"><span class="dot"></span> Vencido hace ${Math.abs(dias)} días</span>`; }
                else if (dias <= 30) { estadoBadge = `<span class="badge warning"><span class="dot"></span> En ${dias} días</span>`; }
                else { estadoBadge = `<span class="badge success"><span class="dot"></span> ${dias} días</span>`; }
                return `<tr>
                  <td><b>${d.patente}</b></td>
                  ...
                  <td class="td-action"><button class="icon-btn" onclick="borrarDocumento(${d.id})"><i class="ti ti-trash"></i></button></td>
                </tr>`;
              }).join("")}
            </tbody>
          </table>
    ...
    <div class="section">
      ...
      ${ots.length === 0 ? `<div class="empty"><i class="ti ti-clipboard-off"></i>Sin OTs ${otFiltroEstado ? 'en este filtro' : 'todavía'}.<br>Tocá <b>Nueva OT</b> para cargar la primera.</div>` : `
        <div class="table-wrap">
          <table>
            <thead><tr>
              <th>OT #</th><th>Fecha</th><th>Vehículo</th><th>Conductor</th>
...
async function abrirOT(otId) {
  ...
  const totalCosto = ot.items.reduce((s, i) => s + (i.costo || 0), 0);
    ...
    <div style="background:var(--surface);border-radius:var(--radius);box-shadow:var(--shadow-lg);max-width:880px;width:100%;max-height:92vh;overflow:auto">
      ...
      <div style="padding:18px 22px">
        <div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:16px">
          ...
          <span class="badge cat">${gs(totalCosto)}</span>
        </div>
          ...
          <div class="table-wrap">
            <table>
              <thead><tr>
                ...
                <th class="center">Estado</th><th>Técnico</th><th class="num">Costo</th><th class="td-action"></th>
              </tr></thead>
              <tbody>
                ${ot.items.map((it, idx) => `
                  <tr>
                    ...
                    <td class="num">
                      <input type="text" value="${it.costo || ''}" placeholder="0"
                        onblur="actualizarItem(${it.id},'costo',this.value)"
                        style="width:100px;padding:4px 8px;border:1px solid var(--border);border-radius:6px;font-size:12px;text-align:right">
                    </td>
                    <td class="td-action">
...
const LABEL_ROL = { admin: "Administrador", operador: "Operador", consulta: "Solo consulta" };
        ...
        <div class="card-body">
          <div class="form-row">
            ...
            <div class="field"><label>Rol</label>
              <select id="u-rol" style="width:160px">
                <option value="operador">Operador (carga datos)</option>
                <option value="admin">Administrador (todo)</option>
                <option value="consulta">Solo consulta (ver)</option>
              </select>
            </div>
          ...
          <p class="hint" style="margin-top:14px">
            ...
            <b>Administrador</b>: acceso total + gestión de usuarios. <b>Operador</b>: carga y edita datos. <b>Solo consulta</b>: únicamente puede ver, no modificar.
          </p>
        </div>
      </div>
...
async function cargarFlotaInicial() {
  if (!confirm("¿Cargar la flota completa desde el Excel del servidor?\n\nEsto agrega los 89 vehículos con sus planes, neumáticos y documentos. Solo hacelo una vez.")) return;
  toast("Cargando flota... esto puede tardar unos segundos", "success");
  const r = await api("/api/cargar_flota_inicial", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
  if (r.ok) {
    toast(r.msg, "success");
    setTimeout(() => renderDashboard(), 1000);
  } else {
    toast(r.msg || "Error al cargar", "error");
  }
}
