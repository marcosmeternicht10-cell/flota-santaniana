...
# ════════════════════════════════════════════════════════════════════════════
#  PDF de DOCUMENTOS por vencer (hasta una fecha)
# ════════════════════════════════════════════════════════════════════════════
def generar_documentos_pdf(documentos, hasta, ruta_salida):
    """
    Genera un PDF con los documentos que vencen hasta cierta fecha (incluye vencidos).
    Ordenados por urgencia. Para imprimir y saber qué renovar.
    'documentos' debe venir con un campo 'dias' (días hasta el vencimiento).
    """
    doc = SimpleDocTemplate(ruta_salida, pagesize=A4,
                            topMargin=15*mm, bottomMargin=15*mm,
                            leftMargin=14*mm, rightMargin=14*mm)
    ...
    titulo = ParagraphStyle("DoTit", parent=styles["Heading1"],
        fontSize=16, textColor=colors.HexColor(AZUL), alignment=TA_CENTER, spaceAfter=2)
    subt = ParagraphStyle("DoSub", parent=normal, fontSize=10, textColor=colors.HexColor(GRIS), alignment=TA_CENTER, spaceAfter=12)
    seccion = ParagraphStyle("DoSec", parent=styles["Heading2"],
        fontSize=12, textColor=colors.HexColor(AZUL), spaceBefore=10, spaceAfter=6)
    small = ParagraphStyle("DoSmall", parent=normal, fontSize=8.5, leading=10)
    micro = ParagraphStyle("DoMicro", parent=normal, fontSize=8, leading=10)

    story = []

    # Encabezado con logo
    cab = [
        Paragraph("<b>LA SANTANIANA</b>", ParagraphStyle("E5", parent=normal, fontSize=12, textColor=colors.HexColor(ROJO))),
        Paragraph("Documentos por Vencer", titulo),
    ]
    logo = _logo_path()
    if logo:
        try:
    ...
    story.append(Paragraph(f"Vencimientos hasta el {hasta} &nbsp;·&nbsp; generado el {datetime.date.today().isoformat()}", subt))

    # Resumen
    vencidos = [d for d in documentos if d.get("dias", 0) < 0]
    por_vencer = [d for d in documentos if d.get("dias", 0) >= 0]
    resumen = Table([[
        Paragraph(f"<b>{len(documentos)}</b><br/><font size='8'>Total a revisar</font>", ParagraphStyle("d1",parent=normal,alignment=TA_CENTER)),
        Paragraph(f"<b><font color='{ROJO}'>{len(vencidos)}</font></b><br/><font size='8'>Ya vencidos</font>", ParagraphStyle("d2",parent=normal,alignment=TA_CENTER)),
        Paragraph(f"<b><font color='{AMARILLO}'>{len(por_vencer)}</font></b><br/><font size='8'>Por vencer</font>", ParagraphStyle("d3",parent=normal,alignment=TA_CENTER)),
    ...
    # Detalle
    story.append(Paragraph("Detalle de documentos", seccion))
    if documentos:
        filas = [[
            Paragraph("<b>Vehículo</b>", small), Paragraph("<b>Tipo</b>", small),
            Paragraph("<b>Nombre</b>", small), Paragraph("<b>Vence</b>", small),
            Paragraph("<b>Estado</b>", small), Paragraph("<b>Proveedor</b>", small),
        ]]
        for d in documentos:
            dias = d.get("dias", 0)
            if dias < 0:
                estado = f"<font color='{ROJO}'>Vencido hace {abs(dias)} d</font>"
            elif dias <= 30:
                estado = f"<font color='{AMARILLO}'>En {dias} días</font>"
            else:
                estado = f"<font color='{VERDE}'>En {dias} días</font>"
            filas.append([
                Paragraph(f"<b>{d.get('patente','')}</b>", micro),
                Paragraph(d.get("tipo",""), micro),
                Paragraph(d.get("nombre") or "—", micro),
                Paragraph(d.get("fecha_vencimiento",""), micro),
                Paragraph(estado, micro),
                Paragraph(d.get("proveedor") or "—", micro),
            ])
        tabla = Table(filas, colWidths=[26*mm, 34*mm, 38*mm, 24*mm, 32*mm, 28*mm])
        tabla.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (-1,0), colors.HexColor(AZUL)),
            ("TEXTCOLOR", (0,0), (-1,0), colors.white),
            ("BOX", (0,0), (-1,-1), 0.5, colors.HexColor("#D9E2EE")),
        ...
        story.append(Paragraph("<i>No hay documentos que venzan en ese período. ¡Todo al día!</i>", small))

    story.append(Spacer(1, 8*mm))
    story.append(Paragraph(
        f"<font size='8' color='#999999'>La Santaniana, Sistema de Gestión de Flota</font>", normal))
...
# ════════════════════════════════════════════════════════════════════════════
#  PDF de PRESUPUESTO de Compras (con firma del Director, para Tesorería)
# ════════════════════════════════════════════════════════════════════════════
def generar_presupuesto_compras_pdf(ot, ruta_salida):
    """
    Genera el documento de presupuesto que arma Compras.
    Lleva: items pedidos, precios cargados por Compras, mecánicos, total,
    y espacio de firma del Director General (para llevar a Tesorería).
    """
    doc = SimpleDocTemplate(ruta_salida, pagesize=A4,
                            topMargin=15*mm, bottomMargin=15*mm,
                            leftMargin=16*mm, rightMargin=16*mm)
    ...
    titulo = ParagraphStyle("PrTit", parent=styles["Heading1"],
        fontSize=17, textColor=colors.HexColor(AZUL), alignment=TA_LEFT, spaceAfter=2)
    sub = ParagraphStyle("PrSub", parent=normal, fontSize=9, textColor=colors.HexColor(GRIS), spaceAfter=2)
    seccion = ParagraphStyle("PrSec", parent=styles["Heading2"],
        fontSize=12, textColor=colors.HexColor(AZUL), spaceBefore=10, spaceAfter=6)
    small = ParagraphStyle("PrSmall", parent=normal, fontSize=9, leading=11)
    ...
    # Encabezado con logo
    cab = [
        Paragraph("<b>LA SANTANIANA</b>", ParagraphStyle("E6", parent=normal, fontSize=13, textColor=colors.HexColor(ROJO))),
        Paragraph("Solicitud de Presupuesto / Compra", titulo),
        Paragraph(f"OT N° {ot['id']} &nbsp;·&nbsp; {coche} &nbsp;·&nbsp; {ot.get('patente','')}", sub),
    ]
    logo = _logo_path()
    if logo:
        try:
            img = Image(logo, width=26*mm, height=18*mm)
            head = Table([[cab, img]], colWidths=[140*mm, 28*mm])
            head.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("ALIGN",(1,0),(1,0),"RIGHT")]))
            story.append(head)
        except Exception:
            for x in cab: story.append(x)
    ...
    veh = f"{ot.get('marca','')} {ot.get('modelo','')}".strip()
    datos = [[
        Paragraph(f"<b>Vehículo:</b> {coche} — {ot.get('patente','')}", small),
        Paragraph(f"<b>Fecha:</b> {datetime.date.today().isoformat()}", small),
    ], [
        Paragraph(f"<b>Marca/Modelo:</b> {veh or '—'}", small),
        Paragraph(f"<b>Km:</b> {int(ot.get('km',0) or 0):,}".replace(",", "."), small),
    ]]
    td = Table(datos, colWidths=[110*mm, 58*mm])
    td.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,-1), colors.HexColor(GRIS_CLARO)),
        ("BOX", (0,0), (-1,-1), 0.5, colors.HexColor("#D9E2EE")),
        ("TOPPADDING", (0,0), (-1,-1), 5), ("BOTTOMPADDING", (0,0), (-1,-1), 5),
    ...
    # Items con materiales y precios
    story.append(Paragraph("Materiales / repuestos solicitados", seccion))
    items = ot.get("items", [])
    filas = [[
        Paragraph("<b>#</b>", small),
        Paragraph("<b>Trabajo</b>", small),
        Paragraph("<b>Material necesario</b>", small),
        Paragraph("<b>Mecánico</b>", small),
        Paragraph("<b>Precio</b>", small),
    ]]
    total = 0
    for i, it in enumerate(items, 1):
        precio = it.get("precio_compras") or 0
        total += precio
        filas.append([
            Paragraph(str(i), small),
            Paragraph(it.get("descripcion", ""), small),
            Paragraph(it.get("material_pedido") or "—", small),
            Paragraph(it.get("tecnico") or "—", small),
            Paragraph(gs(precio), small),
        ])
    filas.append([
        Paragraph("", small), Paragraph("", small), Paragraph("", small),
        Paragraph("<b>TOTAL</b>", small), Paragraph(f"<b>{gs(total)}</b>", small),
    ])
    tabla = Table(filas, colWidths=[8*mm, 56*mm, 50*mm, 28*mm, 26*mm])
    tabla.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,0), colors.HexColor(AZUL)),
        ("TEXTCOLOR", (0,0), (-1,0), colors.white),
        ("ALIGN", (4,0), (4,-1), "RIGHT"),
    ...
    story.append(Spacer(1, 4*mm))
    story.append(Paragraph(
        f"<b>Monto total solicitado a Tesorería: {gs(total)}</b>",
        ParagraphStyle("Tot", parent=normal, fontSize=12, textColor=colors.HexColor(ROJO))))

    # Firmas: Compras y Director General
    story.append(Spacer(1, 22*mm))
    firmas = Table([
        ["_______________________________", "_______________________________"],
        [Paragraph("<font size='9'><b>Compras / Depósito</b></font>", normal),
         Paragraph("<font size='9'><b>Director General</b></font>", normal)],
        [Paragraph("<font size='8' color='#888'>Solicita</font>", normal),
         Paragraph("<font size='8' color='#888'>Autoriza el gasto</font>", normal)],
    ], colWidths=[84*mm, 84*mm])
    firmas.setStyle(TableStyle([
        ("ALIGN", (0,0), (-1,-1), "CENTER"),
        ("TOPPADDING", (0,1), (-1,1), 2),
    ...
    nota = Table([[Paragraph(
        "<font size='8' color='#666'>Este documento autoriza el retiro de fondos en Tesorería una vez firmado por el Director General. "
        "Conservar para rendición.</font>", normal)]], colWidths=[168*mm])
...
# ════════════════════════════════════════════════════════════════════════════
#  PDF de PRESUPUESTO DE TURISMO (para presentar al cliente)
# ════════════════════════════════════════════════════════════════════════════
def generar_presupuesto_turismo_pdf(presupuesto, ruta_salida):
    """Genera el PDF de un presupuesto de turismo, listo para el cliente."""
    doc = SimpleDocTemplate(ruta_salida, pagesize=A4,
                            topMargin=15*mm, bottomMargin=15*mm,
                            leftMargin=16*mm, rightMargin=16*mm)
    styles = getSampleStyleSheet()
    ...
    comp = presupuesto.get("completo", {})
    datos = comp.get("datos", {})
    r = comp.get("resultado", {})

    titulo = ParagraphStyle("TuTit", parent=styles["Heading1"],
        fontSize=18, textColor=colors.HexColor(AZUL), alignment=TA_LEFT, spaceAfter=2)
    sub = ParagraphStyle("TuSub", parent=normal, fontSize=9, textColor=colors.HexColor(GRIS), spaceAfter=2)
    seccion = ParagraphStyle("TuSec", parent=styles["Heading2"],
        fontSize=12, textColor=colors.HexColor(AZUL), spaceBefore=12, spaceAfter=6)
    small = ParagraphStyle("TuSmall", parent=normal, fontSize=9.5, leading=12)

    story = []

    # Encabezado con logo
    cab = [
        Paragraph("<b>LA SANTANIANA</b>", ParagraphStyle("E7", parent=normal, fontSize=13, textColor=colors.HexColor(ROJO))),
        Paragraph("Presupuesto de Turismo", titulo),
        Paragraph(f"Presupuesto N° {presupuesto.get('id','')} &nbsp;·&nbsp; {datetime.date.today().isoformat()}", sub),
    ]
    logo = _logo_path()
    if logo:
        try:
    ...
    datos_viaje = [[
        Paragraph(f"<b>Destino:</b> {presupuesto.get('destino','')}", small),
        Paragraph(f"<b>Cliente:</b> {cliente}", small),
    ], [
        Paragraph(f"<b>Distancia:</b> {int(presupuesto.get('km_total',0) or 0):,} km".replace(",", "."), small),
        Paragraph(f"<b>Duración:</b> {presupuesto.get('dias',1)} día(s), {presupuesto.get('noches',0)} noche(s)", small),
    ], [
        Paragraph(f"<b>Pasajeros:</b> {presupuesto.get('pasajeros',0)}", small),
        Paragraph(f"<b>Vehículo:</b> {presupuesto.get('patente') or '—'}", small),
    ]]
    if datos.get("descripcion"):
        datos_viaje.append([Paragraph(f"<b>Detalle:</b> {datos.get('descripcion')}", small), Paragraph("", small)])
    td = Table(datos_viaje, colWidths=[84*mm, 84*mm])
    td.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,-1), colors.HexColor(GRIS_CLARO)),
        ("BOX", (0,0), (-1,-1), 0.5, colors.HexColor("#D9E2EE")),
        ("TOPPADDING", (0,0), (-1,-1), 5), ("BOTTOMPADDING", (0,0), (-1,-1), 5),
    ...
    # Desglose de costos
    story.append(Paragraph("Desglose de costos", seccion))
    filas = [[Paragraph("<b>Concepto</b>", small), Paragraph("<b>Monto</b>", small)]]
    def fila(concepto, monto):
        filas.append([Paragraph(concepto, small), Paragraph(gs(monto), small)])

    fila(f"Combustible ({r.get('litros_estimados',0)} litros aprox.)", r.get("costo_combustible"))
    fila("Desgaste de cubiertas", r.get("costo_cubiertas"))
    fila("Mantenimiento", r.get("costo_mantenimiento"))
    fila("Chofer" + (" (2 choferes)" if r.get("segundo_chofer") else ""), r.get("costo_chofer"))
    if r.get("costo_viaticos"): fila("Viáticos" + (" (×2)" if r.get("segundo_chofer") else ""), r.get("costo_viaticos"))
    if r.get("costo_comidas"): fila("Comidas" + (" (×2)" if r.get("segundo_chofer") else ""), r.get("costo_comidas"))
    if r.get("costo_hospedaje"): fila("Hospedaje", r.get("costo_hospedaje"))
    if r.get("peajes"): fila("Peajes (local)", r.get("peajes"))
    if r.get("peajes_exterior"): fila("Peajes en el exterior", r.get("peajes_exterior"))
    if r.get("seguro_viaje"): fila("Seguro del viaje", r.get("seguro_viaje"))
    if r.get("tramites"): fila("Trámites / habilitaciones", r.get("tramites"))
    if r.get("otros"): fila("Otros gastos", r.get("otros"))

    tabla = Table(filas, colWidths=[128*mm, 40*mm])
    tabla.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,0), colors.HexColor(AZUL)),
        ("TEXTCOLOR", (0,0), (-1,0), colors.white),
        ("ALIGN", (1,0), (1,-1), "RIGHT"),
    ...
    story.append(Spacer(1, 4*mm))
    tot_filas = [
        [Paragraph("Subtotal", small), Paragraph(gs(r.get("subtotal")), small)],
    ]
    if r.get("monto_imprevistos"):
        tot_filas.append([Paragraph(f"Imprevistos ({r.get('imprevistos_pct',0)}%)", small), Paragraph(gs(r.get("monto_imprevistos")), small)])
    tot_filas += [
        [Paragraph(f"Margen ({r.get('margen_pct',0)}%)", small), Paragraph(gs(r.get("monto_margen")), small)],
        [Paragraph(f"IVA ({r.get('iva_pct',0)}%)", small), Paragraph(gs(r.get("monto_iva")), small)],
    ]
    tt = Table(tot_filas, colWidths=[128*mm, 40*mm])
    ...
    pf = Table([[
        Paragraph("<b>PRECIO TOTAL</b>", ParagraphStyle("pf1", parent=normal, fontSize=14, textColor=colors.white)),
        Paragraph(f"<b>{gs(r.get('precio_final'))}</b>", ParagraphStyle("pf2", parent=normal, fontSize=14, textColor=colors.white, alignment=TA_RIGHT)),
    ]], colWidths=[128*mm, 40*mm])
    pf.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,-1), colors.HexColor(ROJO)),
    ...
    if r.get("precio_por_pasajero"):
        story.append(Spacer(1, 3*mm))
        story.append(Paragraph(
            f"<b>Precio por pasajero ({presupuesto.get('pasajeros',0)} personas): {gs(r.get('precio_por_pasajero'))}</b>",
            ParagraphStyle("pp", parent=normal, fontSize=12, textColor=colors.HexColor(AZUL))))

    # Nota al pie
    story.append(Spacer(1, 12*mm))
    nota = Table([[Paragraph(
        "<font size='8' color='#666'>Presupuesto válido por 15 días. Los precios pueden variar según condiciones del viaje, "
        "fluctuaciones del combustible y disponibilidad. No incluye servicios no detallados.</font>", normal)]], colWidths=[168*mm])
    nota.setStyle(TableStyle([
        ("BOX", (0,0), (-1,-1), 0.5, colors.HexColor("#DDDDDD")),
        ("BACKGROUND", (0,0), (-1,-1), colors.HexColor("#FCFCFC")),
        ("TOPPADDING", (0,0), (-1,-1), 8), ("BOTTOMPADDING", (0,0), (-1,-1), 8),
    ...
    story.append(Spacer(1, 6*mm))
    story.append(Paragraph(
        f"<font size='8' color='#999999'>La Santaniana — Transporte y Turismo · Generado el {datetime.date.today().isoformat()}</font>", normal))
