...
<body>
  <div class="app">
    ...
    <aside class="sidebar">
      ...
      <div class="user-box">
        <div class="user-avatar">{{ usuario[0]|upper if usuario else 'U' }}</div>
        <div class="user-details">
          <div class="user-name" id="user-name">{{ usuario }}</div>
          <div class="user-role-badge {{ rol }}" id="user-role">
            {% if rol == 'admin' %}Administración{% else %}Taller{% endif %}
          </div>
        </div>
        <div class="user-actions">
          ...
          {% endif %}
          <button class="user-btn logout" onclick="cerrarSesion()" title="Cerrar sesión"><i class="ti ti-logout"></i></button>
        </div>
      </div>
      <div class="version">v9.3 · Paraguay Gs.</div>
    </aside>
