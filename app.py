...
# Permisos de escritura: 'consulta' no puede modificar nada
@app.before_request
def proteger_escritura():
    """El rol 'consulta' solo puede leer (GET), no modificar."""
    if request.method in ("POST", "PATCH", "PUT", "DELETE"):
        # Excepciones: login y logout
        if request.path in ("/api/login", "/api/logout"):
            return None
        if session.get("rol") == "consulta":
            return jsonify({"error": "Tu rol es de solo consulta, no podés modificar datos"}), 403
    return None
...
def api_crear_usuario():
    ...
    ok, msg = crear_usuario(d["usuario"], d["password"],
                            d.get("nombre", ""), d.get("rol", "operador"))
...
def api_cargar_flota_inicial():
    """
    Carga la flota completa desde el Excel que esté en la carpeta del proyecto.
    Solo funciona si la base está casi vacía (para no duplicar).
    Útil para la primera carga en la nube.
    """
    ...
    for n in nombres_posibles:
        if os.path.exists(n):
            ruta = n
            break
    # Buscar cualquier .xlsx si no encontró por nombre
    if not ruta:
        for f in os.listdir("."):
    ...
    try:
        from flota_seed import cargar_flota_desde_xlsx, cargar_documentos_desde_xlsx
        # Contar antes
        ...
        return jsonify({"ok": True, "msg": f"Flota cargada: {creados} vehículos nuevos.",
                        "creados": creados, "total": despues})
