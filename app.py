...
def proteger_rutas():
    ...
    if session.get("rol") == "compras":
        permitido = (
            ...
            p.startswith("/api/ots") or          # ver detalle de OTs
            p == "/api/dashboard"
        )
        if not permitido:
            if p.startswith("/api/"):
...
def api_compras_entregar(ot_id):
    """Compras entrega el material con foto de evidencia (obligatoria)."""
    d = request.json or {}
    foto = d.get("foto")
    if not foto:
        return jsonify({"ok": False, "msg": "La foto de evidencia es obligatoria"}), 400
    if len(foto) > 800000:
        return jsonify({"ok": False, "msg": "La foto es muy grande"}), 400
    registrar_entrega_compras(ot_id, foto)
    auditar(f"Registró la entrega de la OT #{ot_id}", "Compras", "Con foto de evidencia",
