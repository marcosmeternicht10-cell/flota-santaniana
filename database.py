...
def obtener_cargas_combustible(vehiculo_id=None, desde=None, hasta=None,
    ...
    """
    Lista las cargas de combustible, calculando el rendimiento de cada una.
    El rendimiento se calcula respecto a la carga ANTERIOR del mismo vehículo:
       rendimiento = (odómetro actual - odómetro anterior) / litros de esta carga
    """
    conn = get_connection()
    sql = """
    ...
    if hasta:
        ...
        params.append(hasta)
    if origen:
        sql += " AND COALESCE(c.origen,'taller') = ?"
    ...
    # Calcular el rendimiento de cada carga respecto a la anterior del mismo vehículo
    ultimo_odo = {}  # vehiculo_id -> último odómetro visto
    for f in filas:
        vid = f["vehiculo_id"]
        odo = float(f.get("odometro") or 0)
        litros = float(f.get("litros") or 0)
        if vid in ultimo_odo and odo > ultimo_odo[vid] and litros > 0:
            km_recorridos = odo - ultimo_odo[vid]
            f["km_recorridos"] = round(km_recorridos)
            f["rendimiento"] = round(km_recorridos / litros, 2)
            f["costo_por_km"] = round(float(f.get("costo_total") or 0) / km_recorridos) if km_recorridos else 0
        else:
...
def _actualizar_consumo_desde_combustible(vehiculo_id):
    """Calcula el rendimiento promedio del vehículo y lo guarda como su consumo (para turismo)."""
    cargas = obtener_cargas_combustible(vehiculo_id=vehiculo_id, limite=1000)
    rendimientos = [c["rendimiento"] for c in cargas if c.get("rendimiento")]
    if rendimientos:
        promedio = sum(rendimientos) / len(rendimientos)
        conn = get_connection()
        conn.execute("UPDATE vehiculos SET consumo_km_litro=? WHERE id=?",
                     (round(promedio, 2), vehiculo_id))
        conn.commit()
        conn.close()
...
def actualizar_carga_combustible(carga_id, datos):
    ...
    def num(clave, por_defecto):
        if clave not in datos or datos[clave] in (None, ""):
            return por_defecto
        try:
            return float(str(datos[clave]).replace(".", "").replace(",", "."))
        except Exception:
            return por_defecto
