...
def obtener_cargas_combustible(vehiculo_id=None, desde=None, hasta=None,
    ...
    sql += " ORDER BY c.vehiculo_id, c.odometro ASC"
...
def comparar_con_referencia(vehiculo_id, consumo_real):
    ...
    Devuelve el desvío en porcentaje y una lectura: si consume más de lo
    esperado hay algo que revisar (filtros, inyectores, presión de neumáticos,
    manejo); si consume mucho menos, lo más probable es que falte cargar
    kilómetros, no que el bus sea milagroso.
    """
    ref = referencia_de_vehiculo(vehiculo_id)
    if not ref or not consumo_real or not consumo_real.get("litros_100km"):
        return None
