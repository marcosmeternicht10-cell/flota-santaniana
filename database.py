...
# ════════════════════════════════════════════════════════════════════════════
def registrar_auditoria(usuario, rol, accion, categoria="", detalle=""):
    """Registra una acción en el historial de auditoría."""
    from datetime import datetime
    try:
        conn = get_connection()
        conn.execute("""
            INSERT INTO auditoria (fecha_hora, usuario, rol, accion, categoria, detalle)
            VALUES (?,?,?,?,?,?)
        """, (datetime.now().isoformat(timespec="seconds"),
              usuario or "", rol or "", accion or "", categoria or "", detalle or ""))
        conn.commit()
        conn.close()
    except Exception:
