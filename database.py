"""
database.py — Gestión de la base de datos SQLite v3.0
...
def inicializar_db():
    ...
    columnas_existentes = [r["name"] for r in c.execute("PRAGMA table_info(vehiculos)").fetchall()]
    for col, ddl in [
        ("chasis",    "TEXT DEFAULT ''"),
        ("n_interno", "TEXT DEFAULT ''"),
        ("asientos",  "INTEGER DEFAULT 0"),
    ...
    cols_corr = [r["name"] for r in c.execute("PRAGMA table_info(correctivos)").fetchall()]
    ...
    cols_items = [r["name"] for r in c.execute("PRAGMA table_info(ot_items)").fetchall()]
