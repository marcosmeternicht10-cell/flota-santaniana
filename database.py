...
def contar_admins_activos(excluir=None):
    """Cuántos administradores activos quedan. Sirve para no permitir que el
    sistema se quede sin nadie que pueda administrarlo."""
    conn = get_connection()
    if excluir:
        ...
        n = conn.execute("""SELECT COUNT(*) AS c FROM usuarios
                            WHERE rol='admin' AND activo=1""").fetchone()["c"]
    conn.close()
    return n
