    ...
    <div class="firma-modal">
      <div class="firma-head">
        <div>
          ...
          <p>Se paga una vez por cada semana en la que el chofer trabajó. Marcá a quiénes les corresponde.</p>
        </div>
        <button class="firma-x" onclick="this.closest('.modal-overlay').remove()"><i class="ti ti-x"></i></button>
      </div>
      ...
      <div style="padding:14px 24px 4px">
        <div class="field" style="max-width:220px">
          <label>Monto del plus (Gs. por semana)</label>
          <input id="plus-monto" type="text" inputmode="numeric" value="${d.monto || 0}">
        </div>
      </div>

      <div class="firma-items">
        ${chs.length === 0
          ? `<div class="empty"><i class="ti ti-user-off"></i>No hay choferes dados de alta todavía.</div>`
          : chs.map(x => `
            <label class="firma-item" style="margin-bottom:6px">
              <input type="checkbox" class="plus-check" data-nombre="${(x.nombre || '').replace(/"/g,'&quot;')}" ${x.tiene_plus ? "checked" : ""} onchange="plusRecalcular()">
              <span class="firma-item-info">
                <span class="firma-item-desc">${x.nombre}</span>
                <span class="firma-item-mat">${x.usuario}</span>
              </span>
            </label>`).join("")}
      </div>

      <div class="firma-acciones">
        <span id="plus-cuenta" style="flex:1;font-size:12.5px;color:var(--muted)"></span>
        <button class="btn btn-ghost" onclick="this.closest('.modal-overlay').remove()">Cancelar</button>
        <button class="btn btn-primary" onclick="guardarConfigPlus(this)"><i class="ti ti-check"></i> Guardar</button>
      </div>
    </div>`;
  overlay.onclick = e => { if (e.target === overlay) overlay.remove(); };
  document.body.appendChild(overlay);
...
function plusRecalcular() {
  const n = document.querySelectorAll(".plus-check:checked").length;
  const lbl = document.getElementById("plus-cuenta");
  if (lbl) lbl.textContent = n === 0
    ? "Ningún chofer cobra plus"
    : `${n} chofer(es) cobran el plus`;
}

async function guardarConfigPlus(btn) {
  const nombres = [...document.querySelectorAll(".plus-check:checked")].map(ch => ch.dataset.nombre);
  const monto = $("#plus-monto").value;
  btn.disabled = true;
  const r = await api("/api/corp/config_plus", {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ monto, choferes: nombres })
  });
  if (r && r.ok) {
    toast(r.msg, "success");
    btn.closest(".modal-overlay").remove();
