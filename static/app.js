...
function panelComb(tab, r, cargas, agrup) {
  ...
  if (tab === "periodo") {
    ...
    return `<div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Período</th><th class="center">Cargas</th><th class="num">Litros</th><th class="num">Costo</th><th style="width:130px"></th></tr></thead>
      <tbody>
        ${r.por_periodo.map(p => `<tr>
          <td><b>${p.periodo}</b></td>
          <td class="center">${p.cargas}</td>
          <td class="num">${fmtL(p.litros)} L</td>
          <td class="num"><b>${fmtNum(p.costo)}</b></td>
        ...
        <tr style="background:var(--info-bg)">
          ...
          <td class="num"><b>Gs. ${fmtNum(r.costo)}</b></td><td></td>
        </tr>
      </tbody></table></div></div>`;
  }
  ...
  if (tab === "chofer") {
    ...
    return `<div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Chofer</th><th class="center">Cargas</th><th class="num">Litros</th><th class="num">Costo</th><th style="width:130px"></th></tr></thead>
      <tbody>${r.por_chofer.map(x => `<tr>
        <td><b>${x.chofer}</b></td>
        <td class="center">${x.cargas}</td>
        <td class="num">${fmtL(x.litros)} L</td>
        <td class="num"><b>${fmtNum(x.costo)}</b></td>
  ...
  if (tab === "bus") {
    ...
    return `<div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Bus</th><th>Patente</th><th class="center">Cargas</th><th class="num">Litros</th>
        <th class="center">Consumo<br><span style="font-weight:400;text-transform:none">L cada 100 km</span></th>
        <th class="center">vs. su motor</th><th class="center"></th><th class="num">Costo</th><th style="width:110px"></th></tr></thead>
  ...
  return `<div class="card"><div class="table-wrap"><table>
    <thead><tr><th>Fecha</th><th>Bus</th><th>Chofer</th><th class="num">Odómetro</th>
      <th class="num">Litros</th><th class="num">Gs./L</th><th class="num">Total</th>
    ...
    <tbody>${cargas.map(c => `<tr>
      ...
      <td class="num">${fmtNum(c.precio_litro)}</td>
...
function panelControl(d) {
  ...
  return `
    <div class="ctrl-hero">
      <div class="ctrl-met ${r.alertas ? "malo" : ""}">
        <div class="ctrl-num">${r.alertas + r.revisar}</div>
        <div class="ctrl-lbl">tanques fuera de lo normal</div>
        <div class="ctrl-sub">${r.alertas} muy fuera · ${r.revisar} fuera</div>
      </div>
      <div class="ctrl-met ${r.litros_de_mas ? "malo" : ""}">
        <div class="ctrl-num">${fmtL(r.litros_de_mas)} L</div>
        <div class="ctrl-lbl">consumidos de más</div>
        <div class="ctrl-sub">≈ Gs. ${fmtNum(r.gs_de_mas)}</div>
      </div>
      <div class="ctrl-met ${chAlto.length ? "medio" : ""}">
        <div class="ctrl-num">${chAlto.length}</div>
        <div class="ctrl-lbl">choferes 10% o más arriba</div>
        <div class="ctrl-sub">del normal de los coches que manejan</div>
      </div>
    ...
    <p class="hint" style="margin:0 0 16px"><i class="ti ti-info-circle"></i>
      ...
      estándar propio (hacen falta ${d.parametros.tanques_min} tanques medidos). Los demás se comparan contra su motor, con más margen.</span></p>

    <div class="card">
      <div class="card-head"><i class="ti ti-gas-station"></i> Tanques que consumieron fuera de lo normal
        <span class="hint" style="margin-left:auto;font-weight:400">tocá el coche para ver su gráfico</span></div>
      ${d.alertas.length ? `<div class="table-wrap"><table>
        <thead><tr><th></th><th>Coche</th><th>Fecha</th><th>Chofer</th>
          <th class="center">Consumo</th><th class="center">Normal del coche</th>
          <th class="center">Desvío</th><th class="num">De más</th></tr></thead>
        <tbody>${d.alertas.slice(0, 60).map(a => `<tr class="${a.nivel === "alerta" ? "ctrl-fila-mal" : ""}">
          <td><span class="ctrl-nivel ${a.nivel}">${nivelTxt[a.nivel]}</span></td>
          <td><a class="ctrl-link" onclick="verGraficoControl(${a.vehiculo_id})"><b>#${a.bus}</b></a></td>
          <td style="white-space:nowrap">${a.fecha}</td>
          <td>${a.ambigua
            ? `<span title="El tanque incluye cargas sin kilometraje: ${a.cargas_sin_km.map(x => x.chofer + " " + x.litros + " L").join(", ")}">
                 ${a.involucrados.join(", ")} <i class="ti ti-users" style="color:var(--warning)"></i></span>
               <div class="ctrl-nota">incluye cargas sin km — no se puede atribuir a uno solo</div>`
            : (a.chofer || "—")}</td>
          <td class="center"><b>${fmt2(a.litros_100km)}</b></td>
          <td class="center" style="color:var(--muted)">${fmt2(a.normal)}${a.estandar === "motor" ? ' <span class="ctrl-nota">(motor)</span>' : ""}</td>
          <td class="center"><span class="desvio ${a.nivel === "bajo" ? "revisar" : a.nivel === "alerta" ? "malo" : "atencion"}">${a.desvio_pct > 0 ? "+" : ""}${a.desvio_pct}%</span></td>
          <td class="num">${a.nivel === "bajo" ? "—" : `${fmtL(a.litros_de_mas)} L<div class="ctrl-nota">Gs. ${fmtNum(a.gs_de_mas)}</div>`}</td>
        </tr>`).join("")}</tbody></table></div>`
        : `<div class="empty"><i class="ti ti-circle-check"></i>Ningún tanque salió de la banda de su coche en este período.</div>`}
    </div>
    ...
    <div class="card">
      ...
      ${d.choferes.length ? `<div class="table-wrap"><table>
        <thead><tr><th>Chofer</th><th class="center">Desvío</th><th class="center">Tramos medidos</th>
          <th class="center">Sin kilometraje</th><th class="center">Tanques fuera</th><th>Coches</th></tr></thead>
        <tbody>${d.choferes.map(x => `<tr>
          <td><b>${x.chofer}</b></td>
          <td class="center">${x.desvio_pct === null ? '<span style="color:var(--light)" title="Hacen falta al menos 2 tramos medidos">—</span>'
        ...
        <p class="hint" style="padding:10px 16px"><i class="ti ti-bulb"></i>
          ...
          Un chofer con muchas cargas sin kilometraje queda fuera del control — esa también es una señal.</span></p>`
    ...
    <div class="card">
      ...
      ${d.buses.length ? `<div class="table-wrap"><table>
        <thead><tr><th>Coche</th><th class="center">Consumo normal</th><th class="center">Banda aceptable</th>
          <th class="center">Su motor dice</th><th class="num">Costo por km</th><th class="center">Fuera</th><th></th></tr></thead>
        <tbody>${d.buses.map(b => `<tr>
          ...
          <td class="num">Gs. ${fmtNum(b.cpk)}</td>
    ...
    <div class="firma-modal" style="max-width:900px">
      ...
      <div style="padding:16px 22px 6px">
        ...
        <div class="ctrl-datos-bus">
          ...
          <div><span>Costo por km</span><b>Gs. ${fmtNum(b.cpk)}</b></div>
