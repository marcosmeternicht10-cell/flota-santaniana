...
async function cargarCombCorp() {
  ...
  const con = r.consumo || { km_l: 0, litros_100km: 0, tramos: 0, km_totales: 0 };
  ...
  cont.innerHTML = `
    <div class="metrics" style="margin-bottom:18px">
      ...
      <div class="metric" title="Se calcula dividiendo el total de kilómetros por el total de litros">
        <div class="metric-content"><div class="metric-label">Consumo</div>
        <div class="metric-value">${con.km_l ? con.km_l.toLocaleString("es-PY", {minimumFractionDigits:2}) : "—"}<span style="font-size:13px;color:var(--muted)"> km/L</span></div>
        <div class="metric-foot">${con.litros_100km ? con.litros_100km.toLocaleString("es-PY", {minimumFractionDigits:2}) + " L cada 100 km" : "faltan datos"}</div></div></div>
    </div>

    ${con.tramos ? `<p class="hint" style="margin:-8px 0 16px"><i class="ti ti-info-circle"></i>
      Consumo medido sobre <b>${fmtNum(con.km_totales)} km</b> y <b>${fmtL(con.litros_totales)} litros</b> en ${con.tramos} tramo(s) de tanque a tanque.</p>` : ""}
...
function panelComb(tab, r, cargas, agrup) {
  ...
  if (tab === "bus") {
    ...
    return `<div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Bus</th><th>Patente</th><th class="center">Cargas</th><th class="num">Litros</th>
        <th class="center">Km/L</th><th class="center">L/100km</th>
      ...
      <tbody>${r.por_bus.map(x => `<tr>
        ...
        <td class="center">${x.cargas}</td>
        <td class="num">${fmtL(x.litros)} L</td>
        <td class="center">${x.rendimiento ? x.rendimiento.toLocaleString("es-PY",{minimumFractionDigits:2}) : "—"}</td>
        <td class="center"><b>${x.litros_100km ? x.litros_100km.toLocaleString("es-PY",{minimumFractionDigits:2}) : "—"}</b></td>
  ...
  return `<div class="card"><div class="table-wrap"><table>
    <thead><tr><th>Fecha</th><th>Bus</th><th>Chofer</th><th class="num">Odómetro</th>
      ...
      <th class="center">Km/L</th>${esAdmin ? '<th class="td-action"></th>' : ""}</tr></thead>
    <tbody>${cargas.map(c => `<tr>
      ...
      <td class="center">${c.rendimiento
        ? `<span title="${c.km_recorridos} km con ${fmtL(c.litros_del_tramo)} L">${c.rendimiento.toLocaleString("es-PY",{minimumFractionDigits:2})}</span>`
        : (c.rendimiento_dudoso
            ? `<span style="color:var(--danger)" title="Valor fuera de rango: revisá el kilometraje">${c.rendimiento_dudoso} ⚠</span>`
            : "—")}</td>
