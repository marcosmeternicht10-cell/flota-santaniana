...
// El buscador de turismo avisa cuando se elige un coche, para autocompletar
// el consumo igual que hacía el select con su onchange.
...
// El buscador avisa al elegir, para que la pantalla fije el coche igual que
// hacía el onchange del select.
...
let _cochesCache = null;

async function cochesParaBuscador() {
  if (!_cochesCache) _cochesCache = await api("/api/vehiculos");
  return _cochesCache;
}

function refrescarCochesBuscador() { _cochesCache = null; }

// Devuelve el HTML del buscador. `id` es el mismo que tenía el select, para
// que el código que lee su .value siga funcionando igual.
...
function bcocheElegir(id, vid) {
  ...
  // Si la pantalla tenía un onchange en el select viejo, se respeta
...
function bcocheCerrar(id) {
  ...
  setTimeout(() => {
    ...
    // Si quedó texto escrito pero no se eligió nada, se limpia para no
    // dejar la sensación de que hay un coche seleccionado.
