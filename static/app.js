...
// ─── Revisar cargas con fechas que no cierran ───
// No borra nada solo: junta lo que conviene mirar antes de liquidar y deja
// que el encargado decida qué hacer con cada cosa.

async function abrirSospechosas() {
  const params = new URLSearchParams();
  const d1 = $("#corp-desde").value, h1 = $("#corp-hasta").value;
  if (d1) params.set("desde", d1);
  if (h1) params.set("hasta", h1);
  const d = await api(`/api/corp/sospechosas?${params.toString()}`);

  const futuras = d.futuras || [], tardias = d.tardias || [], bloques = d.en_bloque || [];
  const hayAlgo = futuras.length || tardias.length || bloques.length;
    ...
    <div class="firma-modal" style="max-width:720px">
      <div class="firma-head">
        <div>
          ...
          <p>Servicios cuyas fechas conviene mirar antes de liquidar</p>
        </div>
        <button class="firma-x" onclick="this.closest('.modal-overlay').remove()"><i class="ti ti-x"></i></button>
      </div>
      ...
      <div class="firma-items" style="max-height:60vh;overflow-y:auto">
        ${!hayAlgo ? `
          <div class="empty"><i class="ti ti-circle-check"></i>
            Todo en orden. Los servicios se reportaron el día que se hicieron.</div>` : ""}

        ${bloques.length ? `
          <div class="sosp-titulo"><i class="ti ti-stack-2"></i> Cargados de una sola vez (${bloques.length})</div>
          <p class="hint" style="margin:0 0 10px">Varios servicios de días distintos cargados en el mismo minuto. Suele ser alguien completando la planilla de memoria.</p>
          ${bloques.map(b => `
            <div class="sosp-item bloque">
              <div class="sosp-cab">
                <b>${b.chofer_nombre}</b>
                <span class="mini-chip rojo">${b.cantidad} servicios · ${b.dias_distintos} días</span>
                <span style="flex:1"></span>
                <span class="sosp-monto">Gs. ${fmtNum(b.monto)}</span>
              </div>
              <div class="sosp-det">Cargados el <b>${b.momento}</b> · servicios del ${b.fechas.join(", ")}</div>
              <button class="btn btn-ghost" style="margin-top:9px;padding:5px 11px;font-size:12px;color:var(--danger)"
                      onclick="borrarSospechosas([${b.ids.join(",")}], this)">
                <i class="ti ti-trash"></i> Eliminar estos ${b.cantidad}
              </button>
            </div>`).join("")}` : ""}

        ${futuras.length ? `
          <div class="sosp-titulo" style="color:var(--danger)"><i class="ti ti-calendar-x"></i> Con fecha futura (${futuras.length})</div>
          <p class="hint" style="margin:0 0 10px">Imposibles: el servicio todavía no pasó.</p>
          ${futuras.map(x => filaSosp(x)).join("")}` : ""}

        ${tardias.length ? `
          <div class="sosp-titulo"><i class="ti ti-clock-exclamation"></i> Reportados tarde (${tardias.length})</div>
          <p class="hint" style="margin:0 0 10px">Se cargaron varios días después del servicio.</p>
          ${tardias.slice(0, 40).map(x => filaSosp(x)).join("")}
          ${tardias.length > 40 ? `<p class="hint">...y ${tardias.length - 40} más.</p>` : ""}` : ""}

        ${d.sin_registro_de_carga ? `
          <p class="hint" style="margin-top:14px"><i class="ti ti-info-circle"></i>
          Hay ${d.sin_registro_de_carga} servicio(s) anteriores a este control, de los que no se sabe cuándo se cargaron.</p>` : ""}
      </div>
...
function filaSosp(x) {
  return `<div class="sosp-item">
    ...
    <div class="sosp-det" style="color:var(--danger)">${x.motivo}</div>
    <button class="btn btn-ghost" style="margin-top:8px;padding:4px 10px;font-size:11.5px;color:var(--danger)"
            onclick="borrarSospechosas([${x.id}], this)"><i class="ti ti-trash"></i> Eliminar</button>
  </div>`;
}
