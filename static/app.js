...
async function cargarRendiciones() {
  ...
  const rends = await api(`/api/corp/rendiciones?${params.toString()}`);
  ...
  const completados = total - suspendidos;
  const pasajeros = rends.reduce((s, r) => s + (r.pasajeros || 0), 0);
  // Km recorridos (final - inicial, solo válidos)
  const km = rends.reduce((s, r) => {
    const d = (r.km_final || 0) - (r.km_inicial || 0);
    return s + (d > 0 && d < 2000 ? d : 0);
  }, 0);

  cont.innerHTML = `
    <div class="metrics" style="margin-bottom:18px">
      <div class="metric"><div class="metric-content"><div class="metric-label">Servicios</div><div class="metric-value">${total}</div></div></div>
      <div class="metric"><div class="metric-content"><div class="metric-label">Completados</div><div class="metric-value" style="color:var(--success)">${completados}</div></div></div>
      ...
      <div class="metric"><div class="metric-content"><div class="metric-label">Pasajeros</div><div class="metric-value">${fmtNum(pasajeros)}</div></div></div>
    </div>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            ...
            <th class="num">Km</th><th class="center">Pasaj.</th><th class="center">Estado</th>
          </tr></thead>
          <tbody>
            ${rends.map(r => {
              ...
              return `<tr class="${r.estado === 'suspendido' ? 'item-en-espera' : ''}">
                ...
                <td class="center">
                  <span class="badge ${r.estado === 'suspendido' ? 'danger' : 'success'}" style="font-size:11px">
                    ${r.estado === 'suspendido' ? 'Suspendido' : 'Completado'}
                  </span>
