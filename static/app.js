...
async function renderCorpEmpresas() {
  ...
  content.innerHTML = `
    <div class="page-header">
      ...
      <p>Cobertura de tramos, costo por cliente y justificación de los servicios variables</p>
    </div>
    <div class="section">
      <div class="card">
        <div class="card-head"><i class="ti ti-filter"></i> Filtros</div>
        <div class="card-body">
          <div class="form-row" style="align-items:flex-end">
            <div class="field" style="min-width:170px">
              <label>Empresa</label>
              <select id="emp-cliente"><option value="">Todas</option></select>
            </div>
            <div class="field"><label>Desde</label><input id="emp-desde" type="date" value="${fmt(hace30)}"></div>
            <div class="field"><label>Hasta</label><input id="emp-hasta" type="date" value="${fmt(hoy)}"></div>
            <div class="field">
              <label>Ver costo por</label>
              <select id="emp-agrup" onchange="cargarControlEmpresa()" style="width:150px">
            ...
            <button class="btn btn-primary" onclick="cargarControlEmpresa()"><i class="ti ti-search"></i> Buscar</button>
            <button class="btn btn-pdf" onclick="descargarControlPDF()"><i class="ti ti-file-type-pdf"></i> PDF</button>
          </div>
        </div>
      </div>
...
function paramsEmpresa() {
  ...
  const cl = $("#emp-cliente").value, d = $("#emp-desde").value, h = $("#emp-hasta").value;
...
async function cargarControlEmpresa() {
  const cont = $("#emp-resultado");
  cont.innerHTML = `<div class="dossier-loading"><div class="dossier-spinner"></div></div>`;
  const d = await api(`/api/corp/control_empresa?${paramsEmpresa().toString()}`);

  // Cargar la lista de empresas una sola vez
  const selCli = $("#emp-cliente");
  if (selCli && selCli.options.length <= 1 && d.clientes) {
    d.clientes.forEach(c => {
      const o = document.createElement("option");
      o.value = c; o.textContent = c + (d.solo_habiles.includes(c) ? " (L a V)" : "");
  ...
  const cobGlobal = totEsp ? Math.round(totCub / totEsp * 1000) / 10 : 0;
  const faltas = cob.flatMap(c => (c.detalle_faltantes || []).map(f => ({ ...f, cliente: c.cliente })))
                    .filter(f => f.situacion !== "suspendido");

  // Cuando se agrupa, las tarjetas muestran el promedio POR PERÍODO. Ver
  // "1841 tramos" no dice nada; "460 por semana" sí se puede leer.
  const nPer = Math.max(1, new Set(cob.flatMap(c => (c.periodos || []).map(p => p.periodo))).size);
  const porPer = agrup === "total" ? 1 : nPer;
  const etiqueta = { dia: "por día", semana: "por semana", mes: "por mes", total: "en el período" }[agrup];
  const prom = v => Math.round(v / porPer);

  cont.innerHTML = `
    <div class="metrics" style="margin-bottom:18px">
      <div class="metric"><div class="metric-content"><div class="metric-label">Cobertura</div>
        <div class="metric-value" style="color:${cobGlobal >= 90 ? "var(--success)" : cobGlobal >= 70 ? "var(--warning)" : "var(--danger)"}">${cobGlobal}%</div>
        <div class="metric-foot">${etiqueta === "en el período" ? "del período" : "promedio general"}</div></div></div>
      <div class="metric"><div class="metric-content"><div class="metric-label">Tramos cubiertos ${etiqueta}</div>
        <div class="metric-value">${prom(totCub)} / ${prom(totEsp)}</div>
        <div class="metric-foot">${agrup === "total" ? `${totEsp} en total` : `${totEsp} en todo el rango`}</div></div></div>
      <div class="metric"><div class="metric-content"><div class="metric-label">Sin cubrir ${etiqueta}</div>
        <div class="metric-value" style="color:var(--danger)">${prom(totFal)}</div>
        <div class="metric-foot">${totFal} en todo el rango</div></div></div>
      <div class="metric"><div class="metric-content"><div class="metric-label">A medias ${etiqueta}</div>
        <div class="metric-value" style="color:var(--warning)">${prom(totPar)}</div>
        <div class="metric-foot">${totPar} en todo el rango</div></div></div>
      <div class="metric"><div class="metric-content"><div class="metric-label">Costo ${etiqueta}</div>
        <div class="metric-value">Gs. ${fmtNum(prom(cos.costo_total))}</div>
        <div class="metric-foot">Gs. ${fmtNum(cos.costo_total)} en total</div></div></div>
    </div>

    ${cob.some(c => (c.tramos_sin_actividad || []).length) ? `
    <div class="card" style="background:var(--info-bg);border:1px solid var(--info-bd)">
      <div class="card-body" style="padding:13px 16px">
        <div style="font-size:13px;color:var(--text);line-height:1.6">
          <i class="ti ti-info-circle" style="color:var(--info)"></i>
          <b>Se controlan solo los tramos que operan de verdad.</b>
          ${cob.map(c => c.tramos_sin_actividad.length
            ? `<br><span style="color:var(--muted)">${c.cliente}: se controlan ${c.tramos_activos} de ${c.tramos_catalogo} tramos del catálogo — ${c.tramos_sin_actividad.length} nunca se reportaron en este período${c.solo_habiles.length ? ` y ${c.solo_habiles.length} corren solo de lunes a viernes` : ""}.</span>`
            : "").join("")}
        </div>
      </div>
    </div>` : ""}
    ...
    ${cob.length ? `
    <div class="card">
      <div class="card-head"><i class="ti ti-checklist"></i> ¿Se reportaron todos los tramos?
        ${agrup !== "total" ? `<span class="badge" style="margin-left:8px">por ${agrup}</span>` : ""}
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Empresa</th>${agrup !== "total" ? "<th>Período</th>" : ""}
            <th class="center">Días</th><th class="center">Esperados</th>
            <th class="center">Cubiertos</th><th class="center">A medias</th><th class="center">Suspendidos</th>
            <th class="center">Sin cubrir</th><th class="center">Cobertura</th></tr></thead>
          <tbody>
            ${cob.map(c => {
              const filas = (agrup !== "total" && c.periodos && c.periodos.length) ? c.periodos : null;
              const linea = (p, esTotal) => `<tr ${esTotal && filas ? 'style="background:var(--surface2);font-weight:600"' : ""}>
                <td>${esTotal ? `<b>${c.cliente}</b>` : (filas ? "" : `<b>${c.cliente}</b>`)}</td>
                ${agrup !== "total" ? `<td style="font-size:12.5px;color:var(--muted)">${esTotal ? "todo el período" : p.periodo}</td>` : ""}
                <td class="center">${p.dias}</td>
                ...
                <td class="center">${p.suspendidos || "—"}</td>
              ...
              if (!filas) return linea(c, true);
              return filas.map(p => linea(p, false)).join("") + linea(c, true);
            }).join("")}
          </tbody>
        </table>
      </div>
    </div>` : `<div class="card"><div class="card-body"><p class="hint"><i class="ti ti-info-circle"></i> Elegí un rango de fechas para ver la cobertura de tramos.</p></div></div>`}

    <div class="card">
      <div class="card-head"><i class="ti ti-cash"></i> Costo por empresa
        ${agrup !== "total" ? `<span class="badge" style="margin-left:8px">por ${agrup}</span>` : ""}
      </div>
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ...
            <tr style="background:var(--info-bg)">
              <td colspan="5" style="text-align:right;font-weight:600">Costo total</td>
              <td class="num"><b>Gs. ${fmtNum(cos.costo_total)}</b></td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="card">
      <div class="card-head"><i class="ti ti-help-circle"></i> Servicios variables (${vars.cantidad}) — Gs. ${fmtNum(vars.monto_total)}
        ${vars.sin_justificar ? `<span class="badge danger" style="margin-left:8px">${vars.sin_justificar} sin justificar</span>` : ""}
      </div>
      ${vars.cantidad === 0
        ? `<div class="card-body"><p class="hint">No hubo servicios fuera de tramo en este período.</p></div>`
        : `<div class="table-wrap"><table>
            <thead><tr><th>Fecha</th><th>Empresa</th><th>Chofer</th><th>¿Qué servicio fue?</th><th class="num">Monto</th></tr></thead>
            <tbody>
              ${vars.variables.map(v => `<tr class="${!(v.observacion || "").trim() && !v.justif_exenta ? "item-en-espera" : ""}">
                <td style="font-size:12.5px;white-space:nowrap">${v.fecha_servicio}</td>
                <td><span class="badge">${v.cliente}</span></td>
                <td style="font-size:12.5px">${v.chofer_nombre}</td>
                <td style="font-size:12.5px">${(v.observacion || "").trim()
                  || (v.justif_exenta
                      ? '<span style="color:var(--muted)">cargado antes de exigir justificación</span>'
                      : '<span style="color:var(--danger)"><i class="ti ti-alert-triangle"></i> sin justificar</span>')}</td>
                <td class="num">${fmtNum(v.monto)}</td>
              </tr>`).join("")}
            </tbody></table></div>`}
    </div>
    ...
    ${faltas.length ? `
    <div class="card" style="border-left:3px solid var(--danger)">
      <div class="card-head"><i class="ti ti-alert-triangle" style="color:var(--danger)"></i> Tramos sin cubrir (${faltas.length})</div>
      <div class="card-body" style="padding-bottom:6px">
        <p class="hint" style="margin-bottom:10px"><i class="ti ti-info-circle"></i> Agrupado por día. Tocá una fila para ver qué tramos fueron.</p>
        ${agruparFaltas(faltas).map((g, i) => `
          <div class="falta-dia" onclick="toggleFalta(${i})">
            <span style="flex:1"><b>${g.fecha}</b> · <span class="badge">${g.cliente}</span></span>
            <span style="font-size:12.5px;color:var(--danger)">${g.faltantes} sin hacer</span>
            ${g.parciales ? `<span style="font-size:12.5px;color:var(--warning);margin-left:10px">${g.parciales} a medias</span>` : ""}
            <i class="ti ti-chevron-down" style="margin-left:10px;color:var(--muted)"></i>
          </div>
          <div class="falta-detalle" id="falta-${i}" style="display:none">
            ${g.items.map(f => `<div class="falta-item">
              <span style="flex:1">${f.tramo}</span>
              <span style="font-size:12px;color:${f.situacion === "faltante" ? "var(--danger)" : "var(--warning)"}">${f.situacion === "faltante" ? "nadie lo reportó" : f.detalle + (f.quien && f.quien !== "—" ? " · " + f.quien : "")}</span>
            </div>`).join("")}
          </div>`).join("")}
      </div>
    </div>` : ""}`;
...
// Los faltantes se agrupan por día y empresa: una lista de 240 filas sueltas
// no se puede leer, pero "el martes faltaron 14 tramos de ADM" sí.
