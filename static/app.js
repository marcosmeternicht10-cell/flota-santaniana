...
function panelComb(tab, r, cargas, agrup) {
  ...
  if (tab === "bus") {
    ...
    return `<div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Bus</th><th>Patente</th><th class="center">Cargas</th><th class="num">Litros</th><th class="center">Km/L</th><th class="num">Costo</th><th style="width:110px"></th></tr></thead>
      <tbody>${r.por_bus.map(x => `<tr>
        <td><b>#${x.bus}</b></td>
        <td style="color:var(--muted)">${x.patente || "—"}</td>
        <td class="center">${x.cargas}</td>
        <td class="num">${fmtL(x.litros)} L</td>
        <td class="center">${x.rendimiento ? `<span class="badge ${x.rendimiento >= 2 ? "success" : "danger"}">${x.rendimiento}</span>` : "—"}</td>
        <td class="num"><b>${fmtNum(x.costo)}</b></td>
  ...
  return `<div class="card"><div class="table-wrap"><table>
    ...
    <tbody>${cargas.map(c => `<tr>
      ...
      <td class="center">${c.rendimiento ? c.rendimiento.toFixed(2) : "—"}</td>
