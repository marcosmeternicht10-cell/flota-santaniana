...
// ════════════════════════════════════════════════════════════════════════
//  INVENTARIO DE REPUESTOS (DEPÓSITO)
// ════════════════════════════════════════════════════════════════════════
...
async function renderRepuestos() {
  ...
  if (repSoloBajos) qs.push("solo_bajos=1");
  ...
  content.innerHTML = `
    <div class="page-header">
      <h1><i class="ti ti-packages"></i> Inventario de Repuestos</h1>
      <p>Depósito · catálogo con ubicación, stock y alertas de mínimo</p>
    </div>
    <div class="section">

      <div class="metrics">
        <div class="metric">
          <div class="metric-icon blue"><i class="ti ti-packages"></i></div>
          <div class="metric-content"><div class="metric-label">Repuestos</div><div class="metric-value">${resumen.total_items}</div></div>
        </div>
        <div class="metric">
          <div class="metric-icon green"><i class="ti ti-cash"></i></div>
          <div class="metric-content"><div class="metric-label">Valor del stock</div><div class="metric-value">${gsSmall(resumen.valor_total)}</div></div>
        </div>
        <div class="metric warning">
          <div class="metric-icon amber"><i class="ti ti-alert-triangle"></i></div>
          <div class="metric-content"><div class="metric-label">Bajo mínimo</div><div class="metric-value">${resumen.cant_bajos}</div></div>
        </div>
        <div class="metric danger">
          <div class="metric-icon red"><i class="ti ti-circle-off"></i></div>
          <div class="metric-content"><div class="metric-label">Sin stock</div><div class="metric-value">${resumen.cant_sin_stock}</div></div>
        </div>
      </div>

      ${resumen.alertas && resumen.alertas.length ? `
        <div class="card" style="border-left:3px solid var(--warning)">
          <div class="card-head"><i class="ti ti-bell"></i> Pendientes de pedido (${resumen.alertas.length})</div>
          <div class="card-body">
            <div class="alert-list">
              ${resumen.alertas.map(a => `
                <div class="alert-row" onclick="abrirRepuesto(${a.id})" style="cursor:pointer">
                  <span class="estado-pill ${a.estado_stock}">${LABEL_ESTADO_STOCK[a.estado_stock]}</span>
                  <b>${a.descripcion}</b>
                  <span style="color:var(--muted)">${a.codigo}</span>
                  <span style="margin-left:auto">${fmtNum(a.stock_actual)} / ${fmtNum(a.stock_minimo)} ${a.unidad}</span>
                </div>`).join("")}
            </div>
          </div>
        </div>
      ...
      ${puedeEditar ? `
      <div class="card">
        <div class="card-head"><i class="ti ti-plus"></i> Agregar repuesto al catálogo</div>
        <div class="card-body">
          <div class="form-row">
            <div class="field"><label>Código fabricante *</label><input id="r-cod" placeholder="MANN-W1374" style="width:160px"></div>
            <div class="field" style="flex:1;min-width:220px"><label>Descripción *</label><input id="r-desc" placeholder="Filtro de aceite Scania DC12"></div>
          ...
          <div class="form-row" style="margin-top:14px">
            ...
            <div class="field"><label>Aplicación</label><input id="r-apl" placeholder="Scania K380" style="width:160px"></div>
          ...
          <div class="form-row" style="margin-top:14px">
            <label style="font-weight:600;font-size:13px;color:var(--muted);align-self:center">Ubicación:</label>
            <div class="field"><label>Pasillo</label><input id="r-up" placeholder="A" style="width:70px;text-transform:uppercase"></div>
            <div class="field"><label>Estantería</label><input id="r-ue" placeholder="02" style="width:80px"></div>
            <div class="field"><label>Nivel</label><input id="r-un" placeholder="03" style="width:70px"></div>
            <div class="field"><label>Posición</label><input id="r-uo" placeholder="04" style="width:70px"></div>
            <div class="field" style="align-self:center"><label>&nbsp;</label><span class="hint" style="white-space:nowrap"><i class="ti ti-map-pin"></i> Queda como A-02-03-04</span></div>
          </div>
          <div class="form-row" style="margin-top:14px">
            <div class="field"><label>Stock inicial</label><input id="r-stk" placeholder="0" style="width:100px"></div>
            <div class="field"><label>Stock mínimo</label><input id="r-min" placeholder="10" style="width:100px"></div>
            ...
            <div class="field"><label>Costo unit. (Gs.)</label><input id="r-cos" placeholder="85000" style="width:150px"></div>
            <button class="btn btn-primary" onclick="guardarRepuesto()"><i class="ti ti-device-floppy"></i> Guardar</button>
          </div>
        </div>
      </div>
      ` : ""}
        ...
        <div class="card-body">
          <div class="form-row" style="align-items:center">
            <div class="field" style="flex:1;min-width:200px">
              <label>Buscar</label>
              <input id="r-search" placeholder="Código, descripción, marca, aplicación..." value="${repBuscar}" oninput="repBuscarDebounce(this.value)">
            </div>
            <div class="field">
              <label>Categoría</label>
              <select id="r-filtro-cat" onchange="repFiltrarCat(this.value)">
                <option value="">Todas</option>
                ${CAT_REPUESTOS.map(c => `<option value="${c}" ${repFiltroCategoria === c ? "selected" : ""}>${c}</option>`).join("")}
              </select>
            </div>
            <div class="field" style="align-self:flex-end">
              <label style="cursor:pointer;display:flex;align-items:center;gap:6px;font-size:14px;white-space:nowrap;padding:10px 0">
                <input type="checkbox" ${repSoloBajos ? "checked" : ""} onchange="repToggleBajos(this.checked)" style="width:16px;height:16px">
                Solo bajo mínimo
              </label>
            </div>
          </div>
      ...
      <div class="section-title"><i class="ti ti-list"></i> Catálogo (${reps.length})</div>
      ${reps.length === 0 ? `<div class="empty"><i class="ti ti-package-off"></i>${repBuscar || repFiltroCategoria || repSoloBajos ? "No hay repuestos con esos filtros." : "Sin repuestos cargados.<br>Agregá el primero arriba."}</div>` : `
        <div class="table-wrap">
          <table>
            <thead><tr>
              <th>Código</th><th>Descripción</th><th>Categoría</th>
              <th>Ubicación</th><th class="num">Stock</th><th class="num">Mín.</th>
              <th class="num">Valor</th><th class="center">Estado</th><th class="td-action"></th>
            </tr></thead>
            <tbody>
              ${reps.map(r => `
                <tr onclick="abrirRepuesto(${r.id})" style="cursor:pointer">
                  <td><b>${r.codigo}</b>${r.codigo_alt ? `<br><span style="color:var(--muted);font-size:11px">${r.codigo_alt}</span>` : ""}</td>
                  <td>${r.descripcion}${r.marca || r.aplicacion ? `<br><span style="color:var(--muted);font-size:11px">${[r.marca, r.aplicacion].filter(Boolean).join(" · ")}</span>` : ""}</td>
                  <td style="color:var(--muted);font-size:12px">${r.categoria}</td>
                  <td>${r.ubicacion ? `<span class="badge"><i class="ti ti-map-pin" style="font-size:11px"></i> ${r.ubicacion}</span>` : `<span style="color:var(--muted)">—</span>`}</td>
                  <td class="num"><b>${fmtNum(r.stock_actual)}</b> <span style="color:var(--muted);font-size:11px">${r.unidad}</span></td>
                  <td class="num" style="color:var(--muted)">${fmtNum(r.stock_minimo)}</td>
                  <td class="num">${gsSmall(r.valor_stock)}</td>
                  <td class="center"><span class="estado-pill ${r.estado_stock}">${LABEL_ESTADO_STOCK[r.estado_stock]}</span></td>
                  <td class="td-action" onclick="event.stopPropagation()">
                    <button class="icon-btn" onclick="abrirRepuesto(${r.id})" title="Ver / mover stock"><i class="ti ti-arrows-exchange"></i></button>
                  </td>
                </tr>`).join("")}
            </tbody>
          </table>
      ...
      <p class="hint" style="margin-top:14px">
        <i class="ti ti-bulb"></i>
        Hacé clic en cualquier repuesto para registrar entradas, salidas o un ajuste de inventario, y ver su historial de movimientos.
      </p>
    </div>`;
}

// Debounce para la búsqueda (no recargar en cada tecla)
let _repBuscarTimer = null;
function repBuscarDebounce(v) {
  repBuscar = v;
  clearTimeout(_repBuscarTimer);
  _repBuscarTimer = setTimeout(() => renderRepuestos(), 350);
}
function repFiltrarCat(v) { repFiltroCategoria = v; renderRepuestos(); }
function repToggleBajos(v) { repSoloBajos = v; renderRepuestos(); }
...
async function guardarRepuesto() {
  ...
  const limpiarGs = s => (s || "0").replace(/\./g, "").replace(/,/g, ".");
  const r = await api("/api/repuestos", {
    ...
    body: JSON.stringify({
      ...
      stock_inicial: parseFloat(limpiarGs($("#r-stk").value)) || 0,
      stock_minimo: parseFloat(limpiarGs($("#r-min").value)) || 0,
      unidad: $("#r-uni").value,
      costo_unitario: parseFloat(limpiarGs($("#r-cos").value)) || 0,
    })
  });
  if (r.ok) {
...
// ─── Modal de detalle / movimientos de un repuesto ───
async function abrirRepuesto(id) {
  ...
  const LABEL_MOV = { entrada: "Entrada", salida: "Salida", ajuste: "Ajuste" };
  const COLOR_MOV = { entrada: "success", salida: "danger", ajuste: "cat" };

  const o = document.createElement("div");
  o.className = "modal-overlay";
  o.style.cssText = "position:fixed;inset:0;background:rgba(10,15,26,0.55);z-index:1500;display:grid;place-items:center;padding:20px;overflow-y:auto";
  o.innerHTML = `
    <div style="background:#fff;border-radius:16px;max-width:700px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3)">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:22px 24px 0">
        <div>
          <h2 style="font-size:18px;color:var(--brand-blue);margin:0;display:flex;align-items:center;gap:8px"><i class="ti ti-package"></i> ${r.descripcion}</h2>
          <div style="color:var(--muted);font-size:13px;margin-top:5px">
            <b>${r.codigo}</b>${r.codigo_alt ? " · " + r.codigo_alt : ""} · ${r.categoria}
            ${r.ubicacion ? ` · <i class="ti ti-map-pin" style="font-size:12px"></i> ${r.ubicacion}` : ""}
          </div>
        </div>
      ...
      <div style="padding:18px 24px 24px">
        ...
        <div class="metrics" style="margin-bottom:18px">
          <div class="metric">
            <div class="metric-icon blue"><i class="ti ti-stack-2"></i></div>
            <div class="metric-content"><div class="metric-label">Stock actual</div><div class="metric-value">${fmtNum(r.stock_actual)} <span style="font-size:13px;color:var(--muted)">${r.unidad}</span></div></div>
          </div>
          <div class="metric">
            <div class="metric-icon amber"><i class="ti ti-alert-triangle"></i></div>
            <div class="metric-content"><div class="metric-label">Mínimo</div><div class="metric-value">${fmtNum(r.stock_minimo)}</div></div>
          </div>
          <div class="metric">
            <div class="metric-icon green"><i class="ti ti-cash"></i></div>
            <div class="metric-content"><div class="metric-label">Valor stock</div><div class="metric-value">${gsSmall(r.valor_stock)}</div></div>
          </div>
        </div>

        ${puedeEntrada ? `
        <div class="card">
          <div class="card-head"><i class="ti ti-arrows-exchange"></i> Registrar movimiento</div>
          <div class="card-body">
            <div class="form-row" style="align-items:flex-end">
              <div class="field">
                ...
                <select id="mv-tipo">
                  ...
                  ${puedeEditar ? `<option value="salida">Salida (−)</option>
                  <option value="ajuste">Ajuste (=)</option>` : ""}
                </select>
              </div>
              <div class="field"><label>Cantidad</label><input id="mv-cant" placeholder="1" style="width:100px"></div>
              <div class="field" style="flex:1;min-width:180px"><label>Motivo</label><input id="mv-mot" placeholder="Compra / uso en coche 47 / rotura"></div>
              <div class="field"><label>Costo unit. (entrada)</label><input id="mv-cos" placeholder="opcional" style="width:140px"></div>
              <button class="btn btn-primary" onclick="guardarMovimiento(${r.id})"><i class="ti ti-check"></i> Registrar</button>
            </div>
            <p class="hint" style="margin-top:10px">
              <i class="ti ti-info-circle"></i>
              <b>Entrada</b>: suma. <b>Salida</b>: resta. <b>Ajuste</b>: fija el stock al número que contaste físicamente.
            </p>
          </div>
        </div>
        ` : ""}

        <div class="section-title"><i class="ti ti-history"></i> Movimientos (${r.movimientos.length})</div>
        ${r.movimientos.length === 0 ? `<div class="empty" style="padding:24px"><i class="ti ti-history"></i>Sin movimientos todavía.</div>` : `
          <div class="table-wrap">
            <table>
              <thead><tr><th>Fecha</th><th class="center">Tipo</th><th class="num">Cant.</th><th>Motivo</th><th>Usuario</th>${puedeEditar ? `<th class="td-action"></th>` : ""}</tr></thead>
              <tbody>
                ${r.movimientos.map(m => `
                  <tr>
                    <td style="font-size:12px">${m.fecha}</td>
                    <td class="center"><span class="badge ${COLOR_MOV[m.tipo]}">${LABEL_MOV[m.tipo]}</span></td>
                    <td class="num"><b>${m.tipo === "salida" ? "−" : m.tipo === "entrada" ? "+" : "="}${fmtNum(m.cantidad)}</b></td>
                    <td style="font-size:13px">${m.motivo || "—"}</td>
                    <td style="color:var(--muted);font-size:12px">${m.usuario || "—"}</td>
                    ${puedeEditar ? `<td class="td-action"><button class="icon-btn" onclick="borrarMovimiento(${m.id}, ${r.id})" title="Eliminar movimiento"><i class="ti ti-trash"></i></button></td>` : ""}
                  </tr>`).join("")}
              </tbody>
            </table>
          </div>
        ...
        ${puedeEditar ? `
          <div style="margin-top:18px;text-align:right">
            <button class="btn btn-ghost" onclick="borrarRepuesto(${r.id}, '${r.codigo.replace(/'/g, "")}')"><i class="ti ti-trash"></i> Dar de baja repuesto</button>
          </div>
        ` : ""}
...
function cerrarRepuesto() {
  document.querySelectorAll(".modal-overlay").forEach(o => {
    if (o.querySelector(".ti-package")) o.remove();
  });
}

async function guardarMovimiento(rid) {
  const tipo = $("#mv-tipo").value;
  const cantStr = ($("#mv-cant").value || "").replace(/\./g, "").replace(/,/g, ".");
  const cant = parseFloat(cantStr);
  if (!cant && cant !== 0) { toast("Ingresá una cantidad", "error"); return; }
  const costo = ($("#mv-cos").value || "0").replace(/\./g, "").replace(/,/g, ".");
  const r = await api(`/api/repuestos/${rid}/movimiento`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      tipo: tipo, cantidad: cant, motivo: $("#mv-mot").value,
  ...
  if (r.ok) {
    ...
    cerrarRepuesto();
    abrirRepuesto(rid);       // reabrir el modal con datos frescos
    const cont = document.getElementById("content");
    if (cont && cont.querySelector(".page-header .ti-packages")) renderRepuestos();
  } else toast(r.msg, "error");
}
...
async function borrarMovimiento(mid, rid) {
  ...
  if (r.ok) { toast("Movimiento eliminado"); cerrarRepuesto(); abrirRepuesto(rid); renderRepuestos(); }
  else toast(r.msg, "error");
}

async function borrarRepuesto(id, cod) {
  if (!confirm(`¿Dar de baja el repuesto ${cod}? Se conserva el historial pero deja de aparecer en el catálogo.`)) return;
